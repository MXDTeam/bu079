...
public class SkilledCommand
{
    ...
    public static class FakeReport extends CommandExecute
    {
        ...
        public boolean execute(final MapleClient c, final String[] splitted) {
            ...
            if (target.isGM()) {
                c.getPlayer().dropMessage(1, "无法黑单GM写");
                return true;
            }
    ...
    public static class WarpT extends CommandExecute
    {
        ...
        public boolean execute(final MapleClient c, final String[] splitted) {
            ...
            final List<MapleCharacter> chrs = (List<MapleCharacter>)new LinkedList();
            ...
            if (chrs.isEmpty()) {
                c.getPlayer().dropMessage(6, "沒有搜寻到名称含有 '" + input + "' 的角色");
            }
        ...
        public boolean execute(final MapleClient c, final String[] splitted) {
            ...
            if (victim != null) {
                ...
                try {
                    ...
                    c.getPlayer().dropMessage(6, "出問题了 " + e.getMessage());
                }
            }
            return true;
        }
    ...
    public static class onlineGM extends CommandExecute
    {
        ...
        public boolean execute(final MapleClient c, final String[] splitted) {
            ...
            int totalOnline = 0;
            int GmInChannel = 0;
            List<MapleCharacter> chrs = (List<MapleCharacter>)new LinkedList();
            for (final MapleCharacter chr : c.getChannelServer().getPlayerStorage().getAllCharactersThreadSafe()) {
                if (chr.getGMLevel() > 0) {
                    ++channelOnline;
                }
            ...
            for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
                ...
                chrs = (List<MapleCharacter>)new LinkedList();
            }
            c.getPlayer().dropMessage(6, "当前频道总计GM线上人数: " + channelOnline);
            c.getPlayer().dropMessage(6, "-------------------------------------------------------------------------------------");
            c.getPlayer().dropMessage(6, "当前服务器GM总计线上人数: " + totalOnline + "个");
            c.getPlayer().dropMessage(6, "-------------------------------------------------------------------------------------");
            return true;
        }
    ...
    public static class 商城DC extends CommandExecute
    {
        ...
        public boolean execute(final MapleClient c, final String[] splitted) {
            ...
            try {
                ...
                c.getPlayer().dropMessage("异常拋出：玩家解卡失败");
            }
            return true;
        }
    ...
    public static class BanStatus extends CommandExecute
    {
        ...
        public boolean execute(final MapleClient c, final String[] splitted) {
            ...
            c.getPlayer().dropMessage("玩家[" + name + "] 账号ID[" + acid + "]是否被封锁: " + (ACbanned ? "是" : "否") + (Systemban ? "(系统自动封锁)" : "") + ", 原因: " + reason);
    ...
    public static class 个人公告 extends CommandExecute
    {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append("!个人公告 <类型> <玩家> <內容>- 给玩家发送公告").toString();
        }
    }
}
