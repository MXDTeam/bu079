...
public class MapleClient
{
    ...
    private static final Lock loginMutex = new ReentrantLock(true);
    ...
    public MapleClient(final MapleAESOFB send, final MapleAESOFB receive, final Channel session) {
        ...
        this.gender = -1;
        this.loginAttempt = 0;
        this.allowedChar = (List<Integer>)new LinkedList();
        this.macs = (Set<String>)new HashSet();
        this.engines = (Map<String, ScriptEngine>)new HashMap();
        this.idleTask = null;
    ...
    public List<String> loadCharacterNames(final int serverId) {
        final List<String> chars = (List<String>)new LinkedList();
        for (final CharNameAndId cni : this.loadCharactersInternal(serverId)) {
            chars.add(cni.name);
        }
        return chars;
    ...
    private List<CharNameAndId> loadCharactersInternal(final int serverId) {
        final List<CharNameAndId> chars = (List<CharNameAndId>)new LinkedList();
    ...
    public int login(final String account, final String password, final boolean isIPBanned) {
        int loginok = 5;
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            try (final PreparedStatement ps = con.prepareStatement("SELECT id, banned, password, salt, macs, 2ndpassword, gm, vip, greason, tempban, gender, SessionIP FROM accounts WHERE name = ?")) {
    ...
    public final byte getLoginState() {
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            this.loggedIn = false;
            FileoutputUtil.outError("logs/资料库异常.txt", (Throwable)e);
            throw new DatabaseException("登入状态获取失败", (Throwable)e);
        }
    }
    ...
    public final void removalTask(final boolean shutdown) {
        try {
            ...
            if (this.player.getMap() != null) {
                ...
                if (((Integer)Start.ConfigValuesMap.get("离线泡点开关")).intValue() > 0 && this.player.getMapId() == ((Integer)Start.ConfigValuesMap.get("离线泡点地图")).intValue()) {
        ...
        try {
             if (this.player != null) {
                ...
                try {
                    ...
                    FileoutputUtil.outError("logs/下线保存数据异常.txt", (Throwable)ex);
            ...
            FileoutputUtil.outError("logs/下线处理异常.txt", (Throwable)ex2);
            FileoutputUtil.outError("logs/下线处理异常.txt", (Throwable)ex2);
        }
    }
    ...
    public final int deleteCharacter(final int cid) {
        ...
        for (final Integer ch : channels) {
            final MapleCharacter chr = ChannelServer.getInstance(ch.intValue()).getPlayerStorage().getCharacterById(cid);
            if (chr != null) {
                ChannelServer.getInstance(ch.intValue()).removePlayer(chr);
            }
        }
            ...
            public void run() {
                try {
                    if (MapleClient.this.getLatency() < 0) {
                        MapleClient.this.closeseesion = true;
    ...
    public static void banMacs(final String[] macs) {
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            final List<String> filtered = (List<String>)new LinkedList();
    ...
    public List<String> loadCharacterNamesByCharId(final int charId) {
        final List<String> chars = (List<String>)new LinkedList();
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            try (final PreparedStatement ps = con.prepareStatement("SELECT id,name FROM characters WHERE accountid= (SELECT accountid FROM characters where id=?)")) {
                ps.setInt(1, charId);
                try (final ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
    ...
    public final void unLockDisconnect() {
        this.getSession().writeAndFlush(MaplePacketCreator.serverNotice(1, "当前账号在別处登入\r\n若不是你本人操作请及时更改密码。"));
    ...
    public List<String> loadCharacterNamesByAccId(final int accId) {
        final List<String> Acc = (List<String>)new LinkedList();
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
    ...
    public List<Integer> loadCharacterIDsByAccId(final int accId) {
        final List<Integer> Acc = (List<Integer>)new LinkedList();
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
    ...
    public List<Integer> loadCharacterIds(final int serverId) {
        final List<Integer> chars = (List<Integer>)new LinkedList();
        for (final CharNameAndId cni : this.loadCharactersInternal(serverId)) {
