...
import handling.channel.MapleGuildRanking.bossLogRankingInfo;
...
public class MaplePacketCreator
{
    ...
    public static final byte[] openWeb(final String web) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static final byte[] dojoWarpUp() {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.DOJO_WARP_UP.getValue());
        mplew.write(0);
        mplew.write(6);
        return mplew.getPacket();
    }
    
    public static final byte[] updatePlayerStats(final Map<MapleStat, Long> stats, final MapleCharacter chr) {
        return updatePlayerStats(stats, false, chr);
    }
    
    public static final byte[] updatePlayerStats(final Map<MapleStat, Long> mystats, final boolean itemReaction, final MapleCharacter chr) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        for (final Entry<MapleStat, Long> statupdate2 : mystats.entrySet()) {
            switch ((MapleStat)statupdate2.getKey()) {
                case SKIN:
                case LEVEL: {
                    mplew.write((statupdate2.getValue()).byteValue());
                    continue;
                }
                case JOB:
                case STR:
                ...
                case AVAILABLEAP:
                case FAME: {
                    mplew.writeShort((statupdate2.getValue()).shortValue());
                    continue;
                }
                case AVAILABLESP: {
                ...
                case PET: {
                    mplew.writeLong((statupdate2.getValue()).intValue());
                    mplew.writeLong((statupdate2.getValue()).intValue());
                    mplew.writeLong((statupdate2.getValue()).intValue());
                    continue;
                }
    ...
    public static byte[] spawnPlayerNPC(final PlayerNPC npc) {
        ...
        mplew.write(255);
        final Integer cWeapon = (Integer)equip.get(Byte.valueOf((byte)(-111)));
        if (cWeapon != null) {
    ...
    public static final byte[] GainEXP_Others(final long gain, final boolean inChat, final boolean white) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.SHOW_STATUS_INFO.getValue());
        mplew.write(3);
        mplew.write((int)(white ? 1 : 0));
    ...
    public static byte[] spawnPlayerMapobject(final MapleCharacter chr) {
        ...
        if(chr.getCombat() <= 10000)
            ...
            xl = "[天灵境]";
        else if(chr.getCombat() > 500000 && chr.getCombat() <= 1000000)
            xl = "[阴阳境]";
        else if(chr.getCombat() > 1000000 && chr.getCombat() <= 5000000)
            xl = "[混元境]";
        else if(chr.getCombat() > 5000000 && chr.getCombat() <= 10000000)
            xl = "[☆武皇☆]";
        else if(chr.getCombat() > 10000000 && chr.getCombat() <= 20000000)
            xl = "[☆武帝☆]";
        else if(chr.getCombat() > 20000000)
            xl = "[☆天尊☆]";
        if (chr.getGuildId() <= 0) {
            mplew.writeMapleAsciiString("[流浪人士] - 战力：" + chr.getCombat() + " - " + xl);
            mplew.writeZeroBytes(6);
        }else {
            final MapleGuild gs = Guild.getGuild(chr.getGuildId());
            if (gs != null) {
                mplew.writeMapleAsciiString("[" + gs.getName() + "] - 战力：" + chr.getCombat() + " - " + xl);
        ...
        final List<Pair<Integer, Integer>> buffvalue = (List<Pair<Integer, Integer>>)new ArrayList();
        final List<Pair<Integer, Integer>> buffvaluenew = (List<Pair<Integer, Integer>>)new ArrayList();
        ...
        if (chr.getBuffedValue(MapleBuffStat.COMBO) != null) {
            ...
            buffvalue.add(new Pair(chr.getBuffedValue(MapleBuffStat.COMBO), Integer.valueOf(1)));
        }
        ...
        if (chr.getBuffedValue(MapleBuffStat.DIVINE_BODY) != null) {
            ...
            buffvalue.add(new Pair(chr.getBuffedValue(MapleBuffStat.DIVINE_BODY), Integer.valueOf(3)));
        }
        ...
        if (chr.getBuffedValue(MapleBuffStat.MORPH) != null) {
            ...
            buffvalue.add(new Pair(chr.getBuffedValue(MapleBuffStat.MORPH), Integer.valueOf(2)));
        }
        for (int i = 0; i < mask.length; ++i) {
            mplew.writeInt(mask[i]);
        }
        for (final Pair j : buffvalue) {
            if (((Integer)j.right).intValue() == 3) {
                mplew.writeInt(((Integer)j.left).intValue());
            }
            else if (((Integer)j.right).intValue() == 2) {
                mplew.writeShort((int)((Integer)j.left).shortValue());
            }
            else {
                if (((Integer)j.right).intValue() != 1) {
                    continue;
                }
        ...
        if (buffSrc > 0) {
            ...
            if (buffSrc == 20001019) {
                mplew.writeInt(GameConstants.getMountItems(chr.getChair()));
            }
            else if (buffSrc == 1013 && c_mount != null) {
                mplew.writeInt(c_mount.getItemId());
            }
            else if (buffSrc == 1013 && mount != null) {
                mplew.writeInt(mount.getItemId());
            }
            else if (GameConstants.getMountItem(buffSrc) == 0 && c_mount != null) {
                mplew.writeInt(c_mount.getItemId());
            }
        ...
        allrings.addAll((Collection<? extends MapleRing>)rings.getRight());
                ...
                if (oned.attack != null) {
                    ...
                    for (final Pair<Integer, Boolean> eachd : oned.attack) {
                        if (((Boolean)eachd.right).booleanValue()) {
                            mplew.writeInt(((Integer)eachd.left).intValue() + Integer.MIN_VALUE);
                        }
                        else {
                            mplew.writeInt(((Integer)eachd.left).intValue());
                        }
    ...
    public static final byte[] charInfo(final MapleCharacter chr, final boolean isSelf) {
        ...
        final List<Integer> medalQuests = (List<Integer>)new ArrayList();
        ...
        while (iterator3.hasNext()) {
            final int x2 = ((Integer)iterator3.next()).intValue();
            mplew.writeShort(x2);
        }
        final MapleInventory iv = chr.getInventory(MapleInventoryType.SETUP);
        final List<Item> chairItems = (List<Item>)new ArrayList();
        ...
        final List<Item> medalsItems = (List<Item>)new ArrayList();
    ...
    public static byte[] giveHoming(final int skillid, final int mobid) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.GIVE_BUFF.getValue());
        final List<MapleBuffStat> st = (List<MapleBuffStat>)new ArrayList();
    ...
    public static byte[] getTradeInvitea(final MapleCharacter c) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.PLAYER_INTERACTION.getValue());
        mplew.write(2);
        mplew.write(6);
    ...
    public static byte[] getTradeStarta(final MapleClient c, final MapleTrade trade, final byte number) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.PLAYER_INTERACTION.getValue());
        mplew.write(5);
        mplew.write(6);
    ...
    public static byte[] getNPCTalkStyle(final int npc, final String talk, final int card, final int[] args) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.NPC_TALK.getValue());
        mplew.write(4);
        mplew.writeInt(npc);
    ...
    public static final byte[] updateQuestMobKills(final MapleQuestStatus status) {
        ...
        while (iterator.hasNext()) {
            final int kills = ((Integer)iterator.next()).intValue();
    ...
    public static byte[] fuckGuildInfo(final MapleCharacter c) {
        ...
        if(c.getCombat() <= 10000)
            ...
            xl = "[天灵境]";
        else if(c.getCombat() > 500000 && c.getCombat() <= 1000000)
            xl = "[阴阳境]";
        else if(c.getCombat() > 1000000 && c.getCombat() <= 5000000)
            xl = "[混元境]";
        else if(c.getCombat() > 5000000 && c.getCombat() <= 10000000)
            xl = "[☆武皇☆]";
        else if(c.getCombat() > 10000000 && c.getCombat() <= 20000000)
            xl = "[☆武帝☆]";
        else if(c.getCombat() > 20000000)
            xl = "[☆天尊☆]";
        mplew.writeMapleAsciiString("[流浪人士] - 战力：" + c.getCombat() + " - " + xl);
        mplew.write(0);
        mplew.writeInt(0);
        mplew.writeInt(0);
        mplew.writeInt(0);
    ...
    public static void getGuildInfo(final MaplePacketLittleEndianWriter mplew, final MapleGuild guild, final MapleCharacter c) {
        ...
        if(c.getCombat() <= 10000)
            ...
            xl = "[☆天尊☆]";
        if (c != null) {
            mplew.writeMapleAsciiString("[" + guild.getName() + "] - 战力：" + c.getCombat() + " - " + xl);
        } else {
            mplew.writeMapleAsciiString("[" + guild.getName() + "] - 战力：" + c.getCombat() + " - " + xl);
        }
        for (int i = 1; i <= 5; ++i) {
    ...
    public static byte[] 勋章(final MapleCharacter chr) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.GUILD_OPERATION.getValue());
        mplew.write(26);
        if (chr == null) {
    ...
    public static byte[] showLevelRanks(final int npcid, final ResultSet rs) throws SQLException {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.GUILD_OPERATION.getValue());
        mplew.write(73);
        mplew.writeInt(npcid);
    ...
    public static byte[] showMesoRanks(final int npcid, final ResultSet rs) throws SQLException {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.GUILD_OPERATION.getValue());
        mplew.write(73);
        mplew.writeInt(npcid);
    ...
    public static byte[] showFameRanks(final int npcid, final ResultSet rs) throws SQLException {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.GUILD_OPERATION.getValue());
        mplew.write(73);
        mplew.writeInt(npcid);
    ...
    public static byte[] showGuildRanks(final int npcid, final ResultSet rs) throws SQLException {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.GUILD_OPERATION.getValue());
        mplew.write(73);
        mplew.writeInt(npcid);
    ...
    public static byte[] boatPacket(final int effect) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.BOAT_EFFECT.getValue());
        mplew.writeShort(effect);
        return mplew.getPacket();
    ...
    public static final byte[] temporaryStats_Aran() {
        final List<Pair<Temp, Integer>> stats = (List<Pair<Temp, Integer>>)new ArrayList();
        stats.add(new Pair(Temp.STR, Integer.valueOf(999)));
        stats.add(new Pair(Temp.DEX, Integer.valueOf(999)));
        stats.add(new Pair(Temp.INT, Integer.valueOf(999)));
        stats.add(new Pair(Temp.LUK, Integer.valueOf(999)));
        stats.add(new Pair(Temp.WATK, Integer.valueOf(255)));
        stats.add(new Pair(Temp.ACC, Integer.valueOf(999)));
        stats.add(new Pair(Temp.AVOID, Integer.valueOf(999)));
        stats.add(new Pair(Temp.SPEED, Integer.valueOf(140)));
        stats.add(new Pair(Temp.JUMP, Integer.valueOf(120)));
    ...
    public static final byte[] temporaryStats_Balrog(final MapleCharacter chr) {
        ...
        stats.add(new Pair(Temp.STR, Integer.valueOf(chr.getStat().getTotalStr() / offset)));
        stats.add(new Pair(Temp.DEX, Integer.valueOf(chr.getStat().getTotalDex() / offset)));
        stats.add(new Pair(Temp.INT, Integer.valueOf(chr.getStat().getTotalInt() / offset)));
        stats.add(new Pair(Temp.LUK, Integer.valueOf(chr.getStat().getTotalLuk() / offset)));
        stats.add(new Pair(Temp.WATK, Integer.valueOf(chr.getStat().getTotalWatk() / offset)));
        stats.add(new Pair(Temp.MATK, Integer.valueOf(chr.getStat().getTotalMagic() / offset)));
        return temporaryStats(stats);
    }
        ...
        for (final Pair<Temp, Integer> statupdate2 : mystats) {
            ...
            if (value.intValue() >= 1) {
                if (value.intValue() <= 512) {
                    mplew.writeShort((int)((Integer)statupdate2.getRight()).shortValue());
                }
                else {
                    mplew.write(((Integer)statupdate2.getRight()).byteValue());
                }
            }
        }
        return mplew.getPacket();
        ...
        for (final HiredMerchant hm : hms) {
            ...
            for (final MaplePlayerShopItem item : items) {
                ...
                if (GameConstants.getInventoryType(itemSearch) == MapleInventoryType.EQUIP) {
                    PacketHelper.addItemInfo3(mplew, item.item);
                }
            }
        }
        return mplew.getPacket();
    ...
    public static byte[] getOwlMessage(final int msg) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter(3);
        mplew.writeShort((int)SendPacketOpcode.SHOP_LINK_RESULT.getValue());
        mplew.write(msg);
        return mplew.getPacket();
    }
    ...
    public static byte[] BeansZJgeidd(final boolean type, final int a) {
        ...
        mplew.write(type ? BeansType.奖励豆豆效果.getType() : BeansType.奖励豆豆效果B.getType());
        mplew.writeInt(a);
        mplew.write(5);
        return mplew.getPacket();
    }
    ...
    public static byte[] BeansZJgeiddB(final int a) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.SHOOT_BEANS.getValue());
        mplew.write(BeansType.奖励豆豆效果B.getType());
        mplew.writeInt(a);
        mplew.write(0);
        return mplew.getPacket();
    }
    ...
    public static byte[] BeansHJG(final byte type) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.SHOOT_BEANS.getValue());
        mplew.write(BeansType.黄金狗.getType());
        mplew.write(type);
        return mplew.getPacket();
    }
    ...
    public static byte[] BeansJDCS(final int a, final int 加速旋转, final int 蓝, final int 绿, final int 红) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.SHOOT_BEANS.getValue());
        mplew.write(BeansType.顏色求进洞.getType());
        mplew.write(a);
        mplew.write(加速旋转);
        mplew.write(蓝);
        mplew.write(绿);
    ...
    public static byte[] BeansJDXZ(final int a, final int 第一排, final int 第三排, final int 第二排, final int 启动打怪效果, final int 中奖率, final int 加速旋转, final boolean 关闭打击效果A, final boolean 关闭打击效果B) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.SHOOT_BEANS.getValue());
        mplew.write(BeansType.进洞旋转.getType());
        mplew.write(a);
        mplew.write(第一排);
        mplew.write(第三排);
        mplew.write(第二排);
    ...
    public static byte[] BeansUP(final int ITEM) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.SHOOT_BEANS.getValue());
        mplew.write(BeansType.领奖npc.getType());
        mplew.writeInt(ITEM);
        return mplew.getPacket();
    }
    ...
    public static byte[] showBeans(final List<MapleBeans> beansInfo) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.SHOOT_BEANS.getValue());
        mplew.write(BeansType.开始打豆豆.getType());
    ...
    public static final byte[] sendMarrageEffect() {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.MARRAGE_EFFECT.getValue());
        return mplew.getPacket();
    }
    
    public static byte[] PVPdamagePlayer(final int chrId, final int type, final int monsteridfrom, final int damage) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.DAMAGE_PLAYER.getValue());
        mplew.writeInt(chrId);
        mplew.write(type);
    ...
    public static byte[] showBossLogRanks(final int npcid, final List<bossLogRankingInfo> all) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.GUILD_OPERATION.getValue());
        mplew.write(73);
        mplew.writeInt(npcid);
        mplew.writeInt(all.size());
        for (final bossLogRankingInfo info : all) {
            mplew.writeMapleAsciiString(info.getName());
            mplew.writeInt(info.getCount());
            mplew.writeInt(0);
            mplew.writeInt(0);
            mplew.writeInt(0);
            mplew.writeInt(0);
    ...
    public static List<MaplePlayerShopItem> searchjyhItem(final int itemSearch) {
        final List<MaplePlayerShopItem> itemz = (List<MaplePlayerShopItem>)new LinkedList();
        for (final MaplePlayerShopItem item : Start.jyhItem) {
    ...
    public static byte[] getjyhss(final int itemSearch, final int pd) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.SHOP_SCANNER_RESULT.getValue());
        mplew.write(6);
        mplew.writeInt(0);
        mplew.writeInt(itemSearch);
        mplew.writeInt(searchjyhItem(itemSearch).size());
        final List<MaplePlayerShopItem> items = searchjyhItem(itemSearch);
        for (final MaplePlayerShopItem item : items) {
            String lx = "";
            switch (item.lx) {
                case 1: {
            ...
            mplew.writeMapleAsciiString(MapleItemInformationProvider.getInstance().getName(item.item.getItemId()));
            mplew.writeInt(1000000000);
            mplew.writeMapleAsciiString(lx);
            mplew.writeInt((int)item.item.getQuantity());
            mplew.writeInt((int)item.bundles);
            mplew.writeInt(item.price);
            mplew.writeInt(item.jyhid);
            mplew.write(pd);
            mplew.write(GameConstants.getInventoryType(itemSearch).getType());
            if (GameConstants.getInventoryType(itemSearch) == MapleInventoryType.EQUIP) {
    ...
    public static byte[] getItemMega(final int channel, final String message, final Item item, final boolean megaEar) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((int)SendPacketOpcode.SERVERMESSAGE.getValue());
        mplew.write(14);
        mplew.writeMapleAsciiString(message);
    ...
    public static void addItemInfo(final MaplePacketLittleEndianWriter mplew, final Item item, final MapleCharacter chr) {
        ...
        mplew.writeInt(item.getItemId());
        final boolean hasUniqueId = item.getUniqueId() > 0 && !GameConstants.isMarriageRing(item.getItemId()) && item.getItemId() / 10000 != 166;
        mplew.write((int)(hasUniqueId ? 1 : 0));
        if (hasUniqueId) {
            mplew.writeLong((long)item.getUniqueId());
        }
        if (item.getPet() != null) {
            PacketHelper.addPetItemInfo(mplew, (IItem)item, item.getPet());
        }
        else {
            ...
            if (item.getType() == 1) {
                ...
                mplew.writeInt((int)equip.getExpPercentage() * 100000);
