...
import constants.WorldConstants.Option;
...
public class LoginPacket
{
    ...
    public static final byte[] getServerList(final Option world) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        for (int j = 1; j <= lastChannel; ++j) {
            int load;
            if (channels.contains(Integer.valueOf(100 * world.getWorld() + j))) {
                load = ChannelServer.getInstance(j).getPlayerStorage().pendingCharacterSize(world.getWorld());
            }
            else {
                load = 1200;
            }
            mplew.writeMapleAsciiString(world.name() + "-" + j);
            mplew.writeInt(190 + load);
            mplew.write(world.getWorld());
