...
public class PacketHelper{
    ...
    public static void addExpirationTime1(final MaplePacketLittleEndianWriter mplew, final long time) {
    ...
    public static final void addItemInfo2(final MaplePacketLittleEndianWriter mplew, final IItem item, final boolean zeroPosition, final boolean leaveOut, final boolean trade) {
        ...
        if (isPet) {
            ...
            mplew.writeShort((int)equip.getFlag());
            mplew.write(equip.getLevel());
            mplew.write((int)equip.getExpPercentage());
            mplew.writeInt(0);
            if (!isCash) {
                mplew.writeLong((long)item.getUniqueId());
            }
    ...
    public static void addItemInfo3(final MaplePacketLittleEndianWriter mplew, final IItem item) {
        addItemInfo3(mplew, item, null);
    }
    
    public static void addItemInfo3(final MaplePacketLittleEndianWriter mplew, final IItem item, final MapleCharacter chr) {
        mplew.write((byte)((item.getPet() != null) ? 3 : item.getType()));
        mplew.writeInt(item.getItemId());
