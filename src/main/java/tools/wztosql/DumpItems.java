...
public class DumpItems
{
    ...
    public DumpItems(final boolean update) throws Exception {
        ...
        this.doneIds = (Set<Integer>)new LinkedHashSet();
        this.hadError = false;
        this.update = false;
        this.id = 0;
        this.update = update;
    ...
    public void dumpItem(final PreparedStatement psa, final PreparedStatement psr, final PreparedStatement ps, final PreparedStatement pse, final MapleData iz) throws Exception {
        ...
        final Map<Integer, Map<String, Integer>> equipStats = (Map<Integer, Map<String, Integer>>)new HashMap();
            ...
            for (final MapleData info : dat) {
                ...
                for (final MapleData data2 : info) {
                    if (data2.getName().length() > 3 && data2.getType() != MapleDataType.CANVAS) {
                        (equipStats.get(Integer.valueOf(lv))).put(data2.getName().substring(3), Integer.valueOf(MapleDataTool.getIntConvert(data2, 0)));
                    }
                }
            }
        }
        ...
        if (dat != null) {
            ...
            final Map<String, Integer> rett = equipStats.get(Integer.valueOf(-1));
        ...
        for (final Entry<Integer, Map<String, Integer>> stats : equipStats.entrySet()) {
            ...
            for (final Entry<String, Integer> stat2 : ((Map<String, Integer>)stats.getValue()).entrySet()) {
                ...
                pse.setInt(4, ((Integer)stat2.getValue()).intValue());
                pse.addBatch();
            }
        }
            ...
            for (final MapleData d2 : dat.getChildren()) {
                ...
                if (z != null) {
                    ...
                    if (z.isElement()) {
                        ...
                        incs = new Pair(Integer.valueOf(Element.getFromChar(ele.charAt(0)).getValue()), Integer.valueOf(Integer.parseInt(ele.substring(1, ele.length()))));
                    }
                    else {
                        incs = new Pair(Integer.valueOf(MapleDataTool.getIntConvert(z.getValue1(), d2, 0)), Integer.valueOf(MapleDataTool.getIntConvert(z.getValue2(), d2, 0)));
                    }
                    psa.setString(2, d2.getName());
                    psa.setInt(3, ((Integer)incs.left).intValue());
                    psa.setInt(4, ((Integer)incs.right).intValue());
                    psa.addBatch();
                }
                else {
