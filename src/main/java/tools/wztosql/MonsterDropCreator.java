...
public class MonsterDropCreator{
    ...
    protected static List<Pair<Integer, String>> itemNameCache = (List<Pair<Integer, String>>)new ArrayList();
    protected static List<Pair<Integer, MobInfo>> mobCache = (List<Pair<Integer, MobInfo>>)new ArrayList();
    protected static Map<Integer, Boolean> bossCache = (Map<Integer, Boolean>)new HashMap();
    ...
    public static void main(final String[] args) throws FileNotFoundException, IOException, NotBoundException, InstanceAlreadyExistsException, MBeanRegistrationException, NotCompliantMBeanException, MalformedObjectNameException {
        ...
        for (final Entry<java.lang.Integer, java.util.List<java.lang.Integer>> e : getDropsNotInMonsterBook().entrySet()) {
            ...
            for (final Integer monsterdrop : (List<Integer>)e.getValue()) {
                final int itemid = monsterdrop.intValue();
                final int monsterId = ((Integer)e.getKey()).intValue();
            ...
            if (dataz.getChildByPath("reward").getChildren().size() > 0) {
                ...
                for (final MapleData drop : dataz.getChildByPath("reward")) {
                    ...
                    for (final Pair Pair : MonsterDropCreator.mobCache) {
                        if (((Integer)Pair.getLeft()).intValue() == monsterId2) {
        ...
        for (final Pair Pair2 : MonsterDropCreator.itemNameCache) {
            if (((Integer)Pair2.getLeft()).intValue() >= 2380000 && ((Integer)Pair2.getLeft()).intValue() <= MonsterDropCreator.lastmonstercardid) {
        ...
        for (final Pair Pair3 : MonsterDropCreator.itemNameCache) {
            if (((Integer)Pair3.getLeft()).intValue() >= 2380000 && ((Integer)Pair3.getLeft()).intValue() <= MonsterDropCreator.lastmonstercardid) {
                bookName.append((String)Pair3.getRight());
                if (bookName.toString().contains((CharSequence)" Card")) {
                    bookName.delete(bookName.length() - 5, bookName.length());
                }
                if (((Integer)Pair3.getLeft()).intValue() == lastmonsterbookid) {
                    continue;
                }
                for (final Pair Pair_2 : MonsterDropCreator.mobCache) {
                    if (((MobInfo)Pair_2.getRight()).getName().equalsIgnoreCase(bookName.toString())) {
                        ...
                        lastmonsterbookid = ((Integer)Pair3.getLeft()).intValue();
                        ++k;
                        break;
                    }
                }
    ...
    private static void getAllItems() {
        ...
        for (final MapleData eqpType : itemsData.getChildren()) {
            for (final MapleData itemFolder2 : eqpType.getChildren()) {
                ...
                itemPairs.add(new Pair(Integer.valueOf(itemId2), itemName2));
            }
        }
