...
public class DumpHairFaceTxt
{
    ...
    public void dumpHairFaceData(final String type) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            if (type.contains((CharSequence)"Hair") && Integer.parseInt(((Integer)list.get(i)).toString().substring(4, 5)) == 0) {
                FileoutputUtil.logToFile("logs/Hair.txt", ((Integer)list.get(i)).toString() + ",");
            }
            if (type.contains((CharSequence)"Face") && Integer.parseInt(((Integer)list.get(i)).toString().substring(2, 3)) == 0) {
                FileoutputUtil.logToFile("logs/Face.txt", ((Integer)list.get(i)).toString() + ",");
            }
        }
    }
}
