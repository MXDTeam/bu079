...
public class MapleStorage implements Serializable
{
    ...
    private MapleStorage(final int id, final byte slots, final int meso, final int accountId) {
        this.typeItems = new EnumMap(MapleInventoryType.class);
        this.changed = false;
        this.id = id;
        this.slots = slots;
    ...
    private List<IItem> filterItems(final MapleInventoryType type) {
        final List<IItem> ret = (List<IItem>)new LinkedList();
        for (final IItem item : this.items) {
    ...
    public void sendStored(final MapleClient c, final MapleInventoryType type) {
        c.sendPacket(MaplePacketCreator.storeStorage(this.slots, type, (Collection<IItem>)this.typeItems.get(type)));
    }
    
    public void sendTakenOut(final MapleClient c, final MapleInventoryType type) {
        c.sendPacket(MaplePacketCreator.takeOutStorage(this.slots, type, (Collection<IItem>)this.typeItems.get(type)));
    }
    ...
    public void update(final MapleClient c) {
        c.sendPacket(MaplePacketCreator.arrangeStorage(this.slots, this.items, true));
    }
