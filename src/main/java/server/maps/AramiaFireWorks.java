...
public class AramiaFireWorks
{
    ...
    public final void giveKegs(final MapleCharacter c, final int kegs) {
        switch (this.kegs += (short)kegs) {
            case 1000: {
                Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "<频道 " + c.getClient().getChannel() + "> 不夜城新年活动进度目前是5000/" + (int)this.kegs + "！！"));
                break;
            }
            case 2000: {
                Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "<频道 " + c.getClient().getChannel() + "> 不夜城新年活动进度目前是5000/" + (int)this.kegs + "！！"));
    ...
    private void broadcastServer(final MapleCharacter c, final int itemid) {
        Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, itemid, "<频道 " + c.getClient().getChannel() + "> 不夜城新年活动即将开始举辦怪物大游行！！"));
    }
    ...
    private void startSun(final MapleMap map) {
        map.startMapEffect("阳光正在綻放！", 5121010);
        for (int i = 0; i < 3; ++i) {
            EventTimer.getInstance().schedule((Runnable)new Runnable() {
                @Override
                public final void run() {
    ...
    private final void startDec(final MapleMap map) {
        map.startMapEffect("圣诞树正在綻放！", 5120000);
        for (int i = 0; i < 3; ++i) {
            EventTimer.getInstance().schedule((Runnable)new Runnable() {
                @Override
                public final void run() {
