...
public class MapleDoor extends AbstractMapleMapObject
{
    ...
    private MaplePortal getFreePortal() {
        final List<MaplePortal> freePortals = (List<MaplePortal>)new ArrayList();
    ...
    public final void sendSpawnData(final MapleClient client) {
        ...
        if (this.target.getId() == client.getPlayer().getMapId() || this.getOwnerId() == client.getPlayer().getId() || (this.getOwner() != null && this.getOwner().getParty() != null && client.getPlayer().getParty() != null && this.getOwner().getParty().getId() == client.getPlayer().getParty().getId())) {
            if (this.getOwner() != null && this.getOwner().getParty() != null && client.getPlayer().getParty() != null && (this.getOwnerId() == client.getPlayer().getId() || this.getOwner().getParty().getId() == client.getPlayer().getParty().getId())) {
                client.sendPacket(MaplePacketCreator.partyPortal(this.town.getId(), this.target.getId(), this.skillId, (this.target.getId() == client.getPlayer().getMapId()) ? this.targetPosition : this.townPortal.getPosition(), this.first));
            }
            ...
            if (this.target.getId() == client.getPlayer().getMapId() || (this.getOwner().getParty() != null && this.getOwner().getParty().getMembers().size() == 1) || this.getOwner().getParty() == null) {
                client.sendPacket(MaplePacketCreator.spawnDoor(this.getOwnerId(), (this.target.getId() == client.getPlayer().getMapId()) ? this.targetPosition : this.townPortal.getPosition(), this.target.getId() == client.getPlayer().getMapId() && this.first));
            }
        }
    }
