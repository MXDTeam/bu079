...
public class MapleTrade
{
    ...
    public final void CompleteTrade() {
        ...
        if (this.exchangeItems != null) {
            ...
            if (this.exchangeMeso - GameConstants.getTaxAmount(this.exchangeMeso) >= 50000000) {
                FileoutputUtil.logToFile("logs/Data/大额金币交易.txt", FileoutputUtil.NowTime() + "角色名字:" + ((MapleCharacter)this.chr.get()).getName() + " 和 " + ((MapleCharacter)partner.chr.get()).getName() + " 交易获得 金币" + (this.exchangeMeso - GameConstants.getTaxAmount(this.exchangeMeso)) + "\r\n");
                Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 大额金币交易 疑似卖游戏币 角色名字:" + ((MapleCharacter)this.chr.get()).getName() + " 和 " + ((MapleCharacter)partner.chr.get()).getName() + " 交易获得 金币" + (this.exchangeMeso - GameConstants.getTaxAmount(this.exchangeMeso))));
            }
            FileoutputUtil.logToFile("logs/Data/交易记录.txt", FileoutputUtil.NowTime() + " 账号角色名字:" + ((MapleCharacter)this.chr.get()).getClient().getAccountName() + " " + ((MapleCharacter)this.chr.get()).getName() + " 和 " + ((MapleCharacter)partner.chr.get()).getClient().getAccountName() + " " + ((MapleCharacter)partner.chr.get()).getName() + " 交易获得 金币" + (this.exchangeMeso - GameConstants.getTaxAmount(this.exchangeMeso)) + " 和 " + this.exchangeItems.size() + "件物品[" + output + "]\r\n");
            Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 角色名字:" + ((MapleCharacter)this.chr.get()).getName() + " 和 " + ((MapleCharacter)partner.chr.get()).getName() + " 交易获得 金币" + (this.exchangeMeso - GameConstants.getTaxAmount(this.exchangeMeso)) + " 和 " + this.exchangeItems.size() + "件物品[" + output + "]"));
        ...
        try {
            ...
            FileoutputUtil.outError("logs/交易存档保存数据异常.txt", (Throwable)ex);
    ...
    public final void chat(final String message) {
        ...
        if (!CommandProcessor.processCommand(((MapleCharacter)this.chr.get()).getClient(), message, CommandType.TRADE)) {
            ...
            if (ServerConfig.LOG_CHAT) {
                FileoutputUtil.logToFile("logs/聊天/交易聊天.txt", " " + FileoutputUtil.NowTime() + " IP: " + ((MapleCharacter)this.chr.get()).getClient().getSession().remoteAddress().toString().split(":")[0] + " 『" + ((MapleCharacter)this.chr.get()).getName() + "』对『" + this.partner.getChr().getName() + "』的交易聊天：  " + message + "\r\n");
            }
            final StringBuilder sb = new StringBuilder("[GM 密语] 『" + ((MapleCharacter)this.chr.get()).getName() + "』对『" + this.partner.getChr().getName() + "』的交易聊天：  " + message);
    ...
    public static final void completeTrade(final MapleCharacter c) {
        ...
        if (partner.isLocked()) {
            ...
            if (local.getChr().getClient().getAccID() == partner.getChr().getClient().getAccID()) {
                ...
                FileoutputUtil.logToFile("logs/Hack/ban/交易异常.txt", "时间: " + FileoutputUtil.NowTime() + " IP: " + local.getChr().getClient().getSessionIPAddress() + " MAC: " + local.getChr().getNowMacs() + " " + local.getChr().getName() + " 和 " + partner.getChr().getName() + " 为同个账号的角色且进行交易\r\n");
    ...
    public static final void inviteTrade(final MapleCharacter c1, final MapleCharacter c2) {
        ...
        if (((Integer)Start.ConfigValuesMap.get("玩家交易")).intValue() < 1) {
    ...
    public static final void inviteCS(final MapleCharacter c1, final MapleCharacter c2) {
        ...
        if (c2 != null && c2.getTrade() == null) {
            ...
            c1.getClient().sendPacket(MaplePacketCreator.serverNotice(5, "另一位玩家已与其他人交易."));
