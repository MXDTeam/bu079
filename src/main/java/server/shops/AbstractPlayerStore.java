...
public abstract class AbstractPlayerStore extends AbstractMapleMapObject implements IMaplePlayerShop
{
    ...
    public AbstractPlayerStore(final MapleCharacter owner, final int itemId, final String desc, final String pass, final int slots) {
        ...
        this.visitors = (List<String>)new LinkedList();
        this.bought = (List<BoughtItem>)new LinkedList();
        this.items = (List<MaplePlayerShopItem>)new LinkedList();
        this.messages = (List<Pair<String, Byte>>)new LinkedList();
    ...
    public boolean saveItems() {
        ...
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            rs.close();
            ps.close();
            final List<Pair<IItem, MapleInventoryType>> iters = (List<Pair<IItem, MapleInventoryType>>)new ArrayList();
    ...
    public List<Pair<Byte, MapleCharacter>> getVisitors() {
        final List<Pair<Byte, MapleCharacter>> chrz = (List<Pair<Byte, MapleCharacter>>)new LinkedList();
        for (byte i = 0; i < this.chrs.length; ++i) {
