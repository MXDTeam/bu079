...
public class HiredMerchant extends AbstractPlayerStore
{
    ...
    public HiredMerchant(final MapleCharacter owner, final int itemId, final String desc) {
        ...
        this.blacklist = (List<String>)new LinkedList();
        ...
        }, (long)(86400000 * ((Integer)Start.ConfigValuesMap.get("雇佣持续时间")).intValue()));
    }
    ...
    public void buy(final MapleClient c, final int item, final short quantity) {
        ...
        if (MapleInventoryManipulator.addFromDrop(c, newItem, false)) {
            ...
            final String itemText = MapleItemInformationProvider.getInstance().getName(newItem.getItemId()) + " (" + (int)perbundle + ") x " + (int)quantity + " 已经被卖出。 剩余数量: " + (int)pItem.bundles + " 购买者: " + c.getPlayer().getName();
            if (Owner != null) {
                Owner.dropMessage(5, "您雇佣商店里面的道具: " + itemText);
            }
            ...
            if (ServerConfig.LOG_MRECHANT) {
                FileoutputUtil.logToFile("logs/Data/精灵商人.txt", "\r\n 时间\u3000[" + FileoutputUtil.NowTime() + "] IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 玩家 " + c.getAccountName() + " " + c.getPlayer().getName() + " 从  " + this.getOwnerName() + " 的精灵商人购买了" + MapleItemInformationProvider.getInstance().getName(newItem.getItemId()) + " (" + newItem.getItemId() + ") x" + (int)quantity + " 单个价钱为 : " + pItem.price);
            }
            final StringBuilder sb = new StringBuilder("[GM 密语] 玩家 " + c.getPlayer().getName() + " 从  " + this.getOwnerName() + " 的精灵商人购买了 " + MapleItemInformationProvider.getInstance().getName(newItem.getItemId()) + "(" + newItem.getItemId() + ") x" + (int)quantity + " 单个价钱为 : " + pItem.price);
