...
public class MapleInventoryManipulator
{
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!GameConstants.isRechargable(itemId)) {
                ...
                while (quantity > 0) {
                    ...
                    if (period > 0L) {
                        nItem.setExpiration(System.currentTimeMillis() + period * (long)(hours ? 1 : 24) * 60L * 60L * 1000L);
                    }
                    if (pet != null) {
                        nItem.setPet(pet);
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, final short quantity, final boolean npcInduced, final boolean cs) {
        ...
        if (((Integer)Start.ConfigValuesMap.get("丢出物品")).intValue() < 1) {
            c.getPlayer().dropMessage(1, "管理员已经从后台禁止丢出道具。");
            c.sendPacket(MaplePacketCreator.enableActions());
            return false;
        }
        try {
