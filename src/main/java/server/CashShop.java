...
public class CashShop implements Serializable
{
    ...
    public void checkExpire(final MapleClient c) {
        final List<IItem> toberemove = (List<IItem>)new ArrayList();
        for (final IItem item : this.inventory) {
    ...
    public List<Pair<IItem, String>> loadGifts() {
        final List<Pair<IItem, String>> gifts = (List<Pair<IItem, String>>)new ArrayList();
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
    ...
    public void save(final Connection con) throws SQLException {
        final List<Pair<IItem, MapleInventoryType>> itemsWithType = (List<Pair<IItem, MapleInventoryType>>)new ArrayList();
        for (final IItem item : this.inventory) {
