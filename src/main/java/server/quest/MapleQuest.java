...
public class MapleQuest implements Serializable
{
    ...
    protected MapleQuest(final int id) {
        this.startReqs = (List<MapleQuestRequirement>)new LinkedList();
        this.completeReqs = (List<MapleQuestRequirement>)new LinkedList();
        this.startActs = (List<MapleQuestAction>)new LinkedList();
        this.completeActs = (List<MapleQuestAction>)new LinkedList();
        this.partyQuestInfo = (Map<String, List<Pair<String, Pair<String, Integer>>>>)new LinkedHashMap();
        this.relevantMobs = (Map<Integer, Integer>)new LinkedHashMap();
    ...
    public static void initQuests() {
        System.out.println("[" + FileoutputUtil.CurrentReadable_Time() + "][信息]:初始化任务系统");
    ...
    public boolean canStart(final MapleCharacter c, final Integer npcid) {
        ...
        for (final MapleQuestRequirement r : this.startReqs) {
            if (r.getType() == MapleQuestRequirementType.dayByDay && npcid != null) {
                this.forceComplete(c, npcid.intValue());
                return false;
            }
            if (!r.check(c, npcid)) {
                return false;
    ...
    public enum MedalQuest
    {
        ...
        冰原雪域山脈探险家(29006, 29012, 50, new int[] { 200000000, 200010100, 200010300, 200080000, 200080100, 211000000, 211030000, 211040300, 211041200, 211041800 }), 
        ...
        米纳爾森林探险家(29011, 29012, 70, new int[] { 240000000, 240010200, 240010800, 240020401, 240020101, 240030000, 240040400, 240040511, 240040521, 240050000 }), 
