...
public class MapleQuestAction implements Serializable
{
    ...
    public MapleQuestAction(final MapleQuestActionType type, final ResultSet rse, final MapleQuest quest, final PreparedStatement pss, final PreparedStatement psq, final PreparedStatement psi) throws SQLException {
        this.intStore = 0;
        this.applicableJobs = (List<Integer>)new ArrayList();
        this.items = null;
        this.skill = null;
        this.state = null;
        this.type = type;
    ...
    private static boolean canGetItem(final QuestItem item, final MapleCharacter c) {
        ...
        if (item.job > 0) {
            ...
            while (iterator.hasNext()) {
                final int codec = ((Integer)iterator.next()).intValue();
            ...
            if (!jobFound && item.jobEx > 0) {
                ...
                while (iterator2.hasNext()) {
                    final int codec2 = ((Integer)iterator2.next()).intValue();
    ...
    public void runStart(final MapleCharacter c, final Integer extSelection) {
        ...
        switch (this.type) {
            ...
            case item: {
                final Map<Integer, Integer> props = (Map<Integer, Integer>)new HashMap();
                ...
                for (final QuestItem item2 : this.items) {
                    ...
                    if (item2.prop != -2) {
                        if (item2.prop == -1) {
                            if (extSelection != null && extSelection.intValue() != extNum++) {
                                continue;
                            }
                        }
            ...
            case skill: {
                for (final Triple<Integer, Integer, Integer> skills : this.skill) {
                    final int skillid = ((Integer)skills.left).intValue();
                    final int skillLevel = ((Integer)skills.mid).intValue();
                    final int masterLevel = ((Integer)skills.right).intValue();
                    final ISkill skillObject = SkillFactory.getSkill(skillid);
                    boolean found = false;
                    final Iterator<Integer> iterator5 = this.applicableJobs.iterator();
                    while (iterator5.hasNext()) {
                        final int applicableJob = ((Integer)iterator5.next()).intValue();
            ...
            case sp: {
                ...
                if (this.applicableJobs.size() > 0) {
                    ...
                    while (iterator6.hasNext()) {
                        final int job_val = ((Integer)iterator6.next()).intValue();
            ...
            case item: {
                ...
                for (final QuestItem item2 : this.items) {
                    ...
                    if (count < 0) {
                        ...
                        if (qusetid == 8630) {
                            Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 任务ID:" + qusetid + " 玩家:" + c.getName() + " 使用10张枫彩票兑换物品:" + name + "ID:" + id));
                        ...
                        if (qusetid == 8631) {
                            Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 任务ID:" + qusetid + " 玩家:" + c.getName() + " 使用50张枫彩票兑换物品:" + name + "ID:" + id));
                        ...
                        if (qusetid == 8632) {
                            Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 任务ID:" + qusetid + " 玩家:" + c.getName() + " 使用100张枫彩票兑换物品:" + name + "ID:" + id));
                        ...
                        if (qusetid == 8633) {
                            Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 任务ID:" + qusetid + " 玩家:" + c.getName() + " 使用200张枫彩票兑换物品:" + name + "ID:" + id));
                        ...
                        if (qusetid == 8634) {
                            Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 任务ID:" + qusetid + " 玩家:" + c.getName() + " 使用500张枫彩票兑换物品:" + name + "ID:" + id));
                        ...
                        if (qusetid == 8635) {
                            Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 任务ID:" + qusetid + " 玩家:" + c.getName() + " 使用1000张枫彩票兑换物品:" + name + "ID:" + id));
                        ...
                        if (qusetid == 8636) {
                            Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 任务ID:" + qusetid + " 玩家:" + c.getName() + " 使用3000张枫彩票兑换物品:" + name + "ID:" + id));
