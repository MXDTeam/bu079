...
public class RankingWorker
{
    ...
    public RankingWorker() {
        this.rankings = (Map<Integer, List<RankingInformation>>)new HashMap();
        this.jobCommands = (Map<String, Integer>)new HashMap();
    }
    ...
    public final Integer getJobCommand(final String job) {
        return (Integer)this.jobCommands.get(job);
    }
    ...
    public final List<RankingInformation> getRankingInfo(final int job) {
        return this.rankings.get(Integer.valueOf(job));
    }
    
    public final void run() {
        System.out.println("[" + FileoutputUtil.CurrentReadable_Time() + "][信息]:初始化服务器排名");
    ...
    private void updateRanking(final Connection con) throws Exception {
        ...
        try (final PreparedStatement charSelect = con.prepareStatement(sb.toString());
            ...
            int rank = 0;
            final Map<Integer, Integer> rankMap = (Map<Integer, Integer>)new LinkedHashMap();
            ...
            while (iterator.hasNext()) {
                ...
                this.rankings.put(Integer.valueOf(i), new ArrayList());
            }
            while (rs.next()) {
                ...
                final int jobRank = ((Integer)rankMap.get(Integer.valueOf(job / 100))).intValue() + 1;
                rankMap.put(Integer.valueOf(job / 100), Integer.valueOf(jobRank));
                ++rank;
                (this.rankings.get(Integer.valueOf(-1))).add(new RankingInformation(rs.getString("name"), job, rs.getInt("level"), rs.getInt("exp"), rank));
                (this.rankings.get(Integer.valueOf(job / 100))).add(new RankingInformation(rs.getString("name"), job, rs.getInt("level"), rs.getInt("exp"), jobRank));
                ps.setInt(1, jobRank);
