...
public class ItemMakerFactory
{
    ...
    protected ItemMakerFactory() {
        this.createCache = (Map<Integer, ItemMakerCreateEntry>)new HashMap();
        this.gemCache = (Map<Integer, GemCreateEntry>)new HashMap();
        System.out.println("[" + FileoutputUtil.CurrentReadable_Time() + "][信息]:初始化锻造系统");
    ...
    public static class GemCreateEntry
    {
        ...
        public GemCreateEntry(final int cost, final int reqLevel, final int reqMakerLevel, final int quantity) {
            this.randomReward = (List<Pair<Integer, Integer>>)new ArrayList();
            this.reqRecipe = (List<Pair<Integer, Integer>>)new ArrayList();
            this.cost = cost;
    ...
    public static class ItemMakerCreateEntry
    {
        ...
        public ItemMakerCreateEntry(final int cost, final int reqLevel, final byte reqMakerLevel, final int quantity, final byte tuc, final int stimulator) {
            this.reqItems = (List<Pair<Integer, Integer>>)new ArrayList();
            this.reqEquips = (List<Integer>)new ArrayList();
            this.cost = cost;
            this.tuc = tuc;
            this.reqLevel = reqLevel;
            this.reqMakerLevel = reqMakerLevel;
