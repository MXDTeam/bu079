...
{
    private static String[] 特殊椅子;
    private static String[] 特殊坐骑;
    private static boolean 特殊椅子开关;
    ...
    public static boolean isJh(final int itemId) {
        return itemId == 2041124 || itemId == 2041125 || itemId == 2041126 || itemId == 2041127 || itemId == 2041120 || itemId == 2041121 || itemId == 2041122 || itemId == 2041123;
    }
    ...
    public static boolean isQHHD(final int itemId) {
        return itemId == 2049116 && itemId / 100 == 20491;
    }
    
    public static int getQHHD(final int itemId) {
        return (itemId == 2049116) ? 10 : 10;
    }
    ...
    public static boolean isZXHD(final int itemId) {
        return itemId == 2049124 || itemId == 2049122;
    }
    
    public static int getZXHD(final int itemId) {
        return (itemId == 2049124) ? 5 : 5;
    }
    
    public static boolean isJRZYHD(final int itemId) {
        return itemId == 2049136 && itemId / 100 == 20491;
    }
    
    public static int getJRZYHD(final int itemId) {
        return (itemId == 2049136) ? 5 : 5;
    }
    
    public static boolean isZYHD(final int itemId) {
        return itemId == 2049129 && itemId / 100 == 20491;
    }
    
    public static int getZYHD(final int itemId) {
        return (itemId == 2049129) ? 10 : 10;
    }
    ...
    public static int Equipment_Bonus_EXP(final int itemid) {
        if (((Integer)Start.ConfigValuesMap.get("道具经验开关")).intValue() > 0) {
            for (int a = 0; a < Start.经验加成表.size(); ++a) {
                if (Integer.parseInt((String)(Start.经验加成表.get(a)).getLeft()) == itemid) {
                    return ((Integer)(Start.经验加成表.get(a)).getRight()).intValue();
                }
            }
        }
        else {
    ...
    public static int getMountItems(final int sourceid) {
        int tamingMob = 1932003;
        if (GameConstants.特殊椅子开关) {
            for (int i = 0; i < GameConstants.特殊椅子.length; ++i) {
                if (sourceid == Integer.parseInt(GameConstants.特殊椅子[i])) {
                    tamingMob = Integer.parseInt(GameConstants.特殊坐骑[i]);
                }
            }
        }
        return tamingMob;
    ...
    public static int getExpRate_Below10(final int job, final MapleCharacter c) {
        int 实际经验 = 0;
        if (((Integer)Start.ConfigValuesMap.get("阶段经验开关")).intValue() > 0) {
            ...
            实际经验 = Integer.parseInt(ServerProperties.getProperty("Lzj.expRate"));
        }
    ...
    public static int getTraitExpNeededForLevel(final int level) {
        if (level < 0 || level >= GameConstants.cumulativeTraitExp.length) {
            return Integer.MAX_VALUE;
        }
        return GameConstants.cumulativeTraitExp[level];
    }
    
    public static boolean isNewMesoPet(final int itemId) {
        boolean rea = false;
        for (int i = 0; i < Start.宠物吸金表.size(); ++i) {
            if (itemId == Integer.parseInt((String)Start.宠物吸金表.get(i))) {
                rea = true;
                break;
            }
        }
    ...
    public static boolean isNewDropPet(final int itemId) {
        boolean rea = false;
        for (int i = 0; i < Start.宠物吸物表.size(); ++i) {
            if (itemId == Integer.parseInt((String)Start.宠物吸物表.get(i))) {
                rea = true;
                break;
            }
        }
    ...
    static {
        特殊椅子 = ServerProperties.getProperty("Lzj.特殊椅子").split(",");
        特殊坐骑 = ServerProperties.getProperty("Lzj.特殊坐骑").split(",");
        特殊椅子开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.特殊椅子开关"));
        ...
        ExpTable = new int[] {0, 15, 34, 57, 92, 135, 372, 560, 840, 1242, 1716,//11级
            2360, 3216, 4200, 5460, 7050, 8840, 11040,13716, 16680, 20216,//21级
            24402, 28980, 34320, 40512, 47216, 54900, 63666, 73080, 83720, 95700,//31级
            108480, 122760, 138666, 155540, 174216, 194832, 216600, 240500, 266682, 294216,//41级
            324240, 356916, 391160, 428280, 468450, 510420, 555680, 604416, 655200, 709716, //51级
            748608, 789631, 832902, 878545, 926689, 977471, 1031036, 1087536, 1147032, 1209994, //61级
            1276301, 1346242, 1420016, 1497832, 1579913, 1666492, 1757815, 1854143, 1955750, 2062925, //71级
            2175973, 2295216, 2420993, 2553663, 2693603, 2841212, 2996910, 3161140, 3334370, 3517093, //81级
            3709829, 3913127, 4127566, 4353756, 4592341, 4844001, 5109452, 5389449, 5684790, 5996316, //91级
            6324914, 6671519, 7037118, 7422752, 7829518, 8258575, 8711144, 9188514, 9692044, 10223168, //101级
            10783397, 11374327, 11997640, 12655110, 13348610, 14080113, 14851703, 15665576, 16524049, 17429566,//111级
            18384706, 19392187, 20454878, 21575805, 22758159, 24005306, 25320796, 26708375, 28171993, 29715818,//121级
            31344244, 33061908, 34873700, 36784778, 38800583, 40926854, 43169645, 45535341, 48030677, 50662758,//131级
            53439077, 56367538, 59456479, 62714694, 66151459, 69776558, 73600313, 77633610, 81887931, 86375389,//141级
            91108760, 96101520, 101367883, 106992842, 112782213, 118962678, 125481832, 132358236, 139611467, 147262175,//151级
            155332142, 163844343, 172823012, 182293713, 192283408, 202820538, 213935103, 225658746, 238024845, 251068606,//161级
            264827165, 279339639, 294647508, 310794191, 327825712, 345790561, 364739883, 384727628, 405810702, 428049128,//171级
            451506220, 476248760, 502347192, 529875818, 558913012, 589541445, 621848316, 655925603, 691870326, 729784819,//181级
            769777027, 811960808, 856456260, 903390063, 952895838, 1005114529, 1060194805, 1118293480, 1179575962, 1244216724, //191级
            1312399800, 1384319309, 1460180007, 1540197871, 1624600714, 1713628833, 1807535693, 1906558648, 2011069705, 2121276324,//201级 
            2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323,//212级
            2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323,//223级
            2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323,//234级
            2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323,//245级
            2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323, 2121276323};//256级

        ClosenessTable = new int[] { 0, 1, 3, 6, 14, 31, 60, 108, 181, 287, 434, 632, 891, 1224, 1642, 2161, 2793, 3557, 4467, 5542, 6801, 8263, 9950, 11882, 14084, 16578, 19391, 22547, 26074, 30000, 30000 };
        ...
        blockedMaps = new int[] { 109050000, 200000112, 200090020, 240060200, 280030000, 280090000, 280030001, 240060201, 900090021, 950101100, 950101010 };
        ...
        goldrewards = new int[] { 3010028, 20, 3010054, 20, 3010126, 15, 3010139, 20, 3010151, 20, 3010193, 20, 3010357, 20, 3010361, 10, 1302013, 20, 1302019, 20, 1302021, 12, 1372037, 11, 1372036, 11, 1302024, 12, 1302031, 20, 1302049, 12, 1302063, 10, 1302080, 12, 1312012, 15, 1322021, 20, 1322023, 20, 1332003, 12, 1332003, 12, 1332015, 12, 1332015, 12, 1332020, 20, 1332021, 12, 1332021, 12, 1372011, 15, 1372011, 15, 1372017, 15, 1372016, 10, 1372016, 10, 1382007, 13, 1382007, 13, 1382008, 10, 1382008, 10, 1382014, 15, 1
 3015245, 5, 3015258, 5, 3015240, 8, 3015238, 8, 3015235, 8, 3012030, 5, 3012024, 5, 3010946, 5, 3010760, 8, 1122003, 10, 1122003, 10, 1122004, 10, 1122004, 10, 1402014, 5, 1002728, 50, 1002850, 3, 1050127, 10, 1072275, 10, 1072368, 10, 2049000, 10, 1082002, 50, 1082145, 50, 1082146, 30, 1082147, 30, 1082148, 30, 1082149, 5, 1082150, 50, 1082175, 40, 1082176, 40, 1082177, 40, 1082178, 40, 1082179, 2, 1102084, 5, 1102085, 10, 1102086, 10, 1102040, 10, 1102041, 10, 1102042, 5, 1102043, 10, 1002939, 5, 1402037, 1, 1112440, 20, 1112441, 20, 1112442, 20, 1112443, 20, 1112661, 10, 1112584, 10, 1112713, 10, 1112444, 25, 1112444, 15, 2100010, 50, 2100016, 50, 2100017, 50, 2100009, 20, 1072239, 5 };
        ...
        eventCommonReward = new int[] { 0, 40, 1, 10, 4031019, 5, 4280000, 3, 4280001, 4, 5490000, 3, 5490001, 4 };
        eventUncommonReward = new int[] { 2, 4, 3, 4, 5160000, 5, 5160001, 5, 5160002, 5, 5160003, 5, 5160004, 5, 5160005, 5, 5160006, 5, 5160007, 5, 5160008, 5, 5160009, 5, 5160010, 5, 5160011, 5, 5160012, 5, 5160013, 5, 4001137, 5, 4001137, 5, 4080000, 5, 4080001, 5, 4080002, 5, 4080003, 5, 4080004, 5, 4080005, 5, 4080006, 5, 4080007, 5, 4080008, 5, 4080009, 5, 4080010, 5, 4080011, 5, 4080100, 5, 4031019, 5, 5121003, 5, 5150042, 5, 2022463, 5, 2022463, 5, 2450000, 2 };
        eventRareReward = new int[] { 4031019, 5, 2049100, 5, 4001137, 10, 2049301, 20, 2049400, 3, 2340000, 1, 3010130, 5, 3010131, 5, 3010132, 5, 3010133, 5, 3010136, 5, 3010116, 5, 3010117, 5, 3010118, 5, 1112405, 1, 1112413, 1, 1112414, 1, 2040211, 1, 2040212, 1, 2049000, 2, 2049001, 2, 2049002, 2, 2049003, 2, 1012058, 2, 1012059, 2, 1012060, 2, 1012061, 2 };
        eventSuperReward = new int[] { 4031019, 5, 4031307, 50, 3010127, 10, 3010128, 10, 3010137, 10, 4001137, 10, 1012139, 10, 1012140, 10, 1012141, 10 };
        fishingReward = new int[] { 0, 40, 1, 40, 1302021, 5, 1072238, 1, 1072239, 1, 2049100, 1, 1302000, 3, 1442011, 1, 4031627, 2, 4031628, 1, 4031630, 1, 4031631, 1, 4031632, 1, 4031633, 2, 4031634, 1, 4031635, 1, 4031636, 1, 4031637, 2, 4031638, 2, 4031639, 1, 4031640, 1, 4031641, 2, 4031642, 2, 4031643, 1, 4031644, 1, 4031645, 2, 4031646, 2, 4031647, 1, 4031648, 1, 4031629, 1, 1102041, 1, 1102042, 1, 2101120, 1 };
        ...
        owlItems = new int[] { 1082002, 2070005, 2070006, 1022047, 1102041, 2044705, 2340000, 2040017, 1092030, 2040804 };
        lBalloon = Arrays.asList(new Balloon("欢迎来到" + LoginServer.getServerName(), 236, 122), new Balloon("禁止开外挂", 0, 276), new Balloon("游戏愉快", 196, 263));
    }
}
