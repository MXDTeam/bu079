...
public class 捉鬼任务线程
{
    ...
    public static void 开启进阶BOSS线程() {
        开启进阶BOSS线程伤害();
        if (捉鬼任务线程.进阶BOSS线程 == null) {
            捉鬼任务线程.进阶BOSS线程 = BuffTimer.getInstance().register((Runnable)new Runnable() {
                @Override
                public void run() {
    ...
    public static void 关闭进阶BOSS线程() {
        if (捉鬼任务线程.进阶BOSS线程 != null) {
            关闭进阶BOSS线程伤害();
            捉鬼任务线程.进阶BOSS线程.cancel(false);
            捉鬼任务线程.进阶BOSS线程 = null;
        }
    }
    
    public static void 开启进阶BOSS线程伤害() {
        if (捉鬼任务线程.进阶BOSS线程伤害 == null) {
            捉鬼任务线程.进阶BOSS线程伤害 = BuffTimer.getInstance().register((Runnable)new Runnable() {
                @Override
                public void run() {
            ...
            }, (long)(1000 * 捉鬼任务线程.伤害间隔时间));
        }
    }
    
    public static void 关闭进阶BOSS线程伤害() {
        if (捉鬼任务线程.进阶BOSS线程伤害 != null) {
            捉鬼任务线程.进阶BOSS线程伤害.cancel(false);
            捉鬼任务线程.进阶BOSS线程伤害 = null;
        }
    }
    ...
    public static void 直接驱散() {
        for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
            for (final MapleCharacter chr : cserv.getPlayerStorage().getAllCharacters()) {
                ...
                if (chr.getMapId() != 捉鬼任务线程.地图 || chr.isGM()) {
                    continue;
                }
    ...
    public static void 减少血量() {
        for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
            for (final MapleCharacter chr : cserv.getPlayerStorage().getAllCharacters()) {
                ...
                if (总概率 > 成功率) {
                    final int 血量 = (int)Math.ceil(Math.random() * (double)(捉鬼任务线程.最大伤害扣血数值 - 捉鬼任务线程.最小伤害扣血数值)) + 捉鬼任务线程.最小伤害扣血数值;
    ...
    public static void 减少蓝量() {
        for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
            for (final MapleCharacter chr : cserv.getPlayerStorage().getAllCharacters()) {
                ...
                if (总概率 > 成功率) {
                    final int 蓝量 = (int)Math.ceil(Math.random() * (double)(捉鬼任务线程.最大伤害扣蓝数值 - 捉鬼任务线程.最小伤害扣蓝数值)) + 捉鬼任务线程.最小伤害扣蓝数值;
    ...
    static {
        ...
        进阶红蜗牛长老 = 9500337;
        飞鱼 = 2230107;
        地图 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务所在地图"));
        坐标 = new Point(232, 185);
        全图掉血开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.捉鬼任务全图掉血开关"));
        全图掉蓝开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.捉鬼任务全图掉蓝开关"));
        减少血量开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.捉鬼任务减少血量开关"));
        全图封锁开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.捉鬼任务全图封锁开关"));
        全图黑暗开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.捉鬼任务全图黑暗开关"));
        全图虚弱开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.捉鬼任务全图虚弱开关"));
        全图诅咒开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.捉鬼任务全图诅咒开关"));
        全图诱导开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.捉鬼任务全图诱导开关"));
        直接死亡开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.捉鬼任务直接死亡开关"));
        直接驱散开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.捉鬼任务直接驱散开关"));
        减少蓝量开关 = Boolean.parseBoolean(ServerProperties.getProperty("Lzj.捉鬼任务减少蓝量开关"));
        互相残杀开关 = false;
        伤害间隔时间 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务伤害间隔时间"));
        技能间隔时间 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务技能间隔时间"));
        最大伤害扣血数值 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务伤害最大扣血数值"));
        最大伤害扣蓝数值 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务伤害最大扣蓝数值"));
        最小伤害扣血数值 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务伤害最小扣血数值"));
        最小伤害扣蓝数值 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务伤害最小扣蓝数值"));
        战士职业回避率 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务战士职业伤害回避率"));
        法师职业回避率 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务法师职业伤害回避率"));
        射手职业回避率 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务射手职业伤害回避率"));
        飞侠职业回避率 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务飞侠职业伤害回避率"));
        海盗职业回避率 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务海盗职业伤害回避率"));
        战士职业技能回避率 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务战士职业技能回避率"));
        法师职业技能回避率 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务法师职业技能回避率"));
        射手职业技能回避率 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务射手职业技能回避率"));
        飞侠职业技能回避率 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务飞侠职业技能回避率"));
        海盗职业技能回避率 = Integer.parseInt(ServerProperties.getProperty("Lzj.捉鬼任务海盗职业技能回避率"));
    }
}
