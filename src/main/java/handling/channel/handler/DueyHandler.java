...
public class DueyHandler
{
        ...
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            try (final PreparedStatement ps = con.prepareStatement("INSERT INTO dueypackages (RecieverId, SenderName, Mesos, TimeStamp, Checked, Type) VALUES (?, ?, ?, ?, ?, ?)", 1)) {
                ...
                try (final ResultSet rs = ps.getGeneratedKeys()) {
                    if (rs.next()) {
                        ItemLoader.DUEY.saveItems(Collections.singletonList(new Pair(item, GameConstants.getInventoryType(item.getItemId()))), Integer.valueOf(rs.getInt(1)));
                    }
                    rs.close();
                }
                ps.close();
    ...
    public static final List<MapleDueyActions> loadItems(final MapleCharacter chr) {
        final List<MapleDueyActions> packages = (List<MapleDueyActions>)new LinkedList();
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
    ...
    public static final void reciveMsg(final MapleClient c, final int recipientId) {
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            try (final PreparedStatement ps = con.prepareStatement("UPDATE dueypackages SET Checked = 0 where RecieverId = ?")) {
                ps.setInt(1, recipientId);
                ps.executeUpdate();
                ps.close();
            }
    ...
    private static void removeItemFromDB(final int packageid, final int charid) {
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            try (final PreparedStatement ps = con.prepareStatement("DELETE FROM dueypackages WHERE PackageId = ? and RecieverId = ?")) {
                ps.setInt(1, packageid);
                ps.setInt(2, charid);
                ps.executeUpdate();
                ps.close();
