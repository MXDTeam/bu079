...
public class InventoryHandler
{
    ...
    public static final void ItemSort(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (nowTimestamp - c.getPlayer().整理背包冷却 > 3000L) {
            c.getPlayer().updateTick(slea.readInt());
    ...
    public static final void ItemGather(final LittleEndianAccessor slea, final MapleClient c) {
        final long nowTimestamp = System.currentTimeMillis();
        if (nowTimestamp - c.getPlayer().集合背包冷却 > 3000L) {
            ...
            final List<IItem> itemMap = (List<IItem>)new LinkedList();
            for (final IItem item : Inv.list()) {
                itemMap.add(item.copy());
            }
            排序並交換位置(itemMap, invType, c);
            c.sendPacket(MaplePacketCreator.finishedGather((int)mode));
            c.sendPacket(MaplePacketCreator.enableActions());
            itemMap.clear();
            c.getPlayer().集合背包冷却 = nowTimestamp;
        }
        else {
            c.getPlayer().dropMessage(1, "请稍等片刻再整理背包。");
            NPCScriptManager.getInstance().dispose(c);
    ...
    private static List<IItem> sortItems(final List<IItem> passedMap) {
        ...
        final List<IItem> sortedList = (List<IItem>)new LinkedList();
        for (final Integer val : itemIds) {
            for (final IItem item2 : passedMap) {
                if (val.intValue() == item2.getItemId()) {
    ...
    private static List<IItem> 排序並交換位置(final List<IItem> passedMap, final MapleInventoryType Type, final MapleClient c) {
        final List<Integer> itemIds = (List<Integer>)new ArrayList();
        for (final IItem item : passedMap) {
            itemIds.add(Integer.valueOf(item.getItemId()));
        }
        Collections.sort(itemIds);
        final List<IItem> sortedList = (List<IItem>)new LinkedList();
        for (short i = 0; i < itemIds.size(); ++i) {
            final short 物品1位置 = (short)(i + 1);
            short 物品2位置 = 0;
            boolean 位置交換 = false;
            for (final IItem item2 : passedMap) {
                if (((Integer)itemIds.get((int)i)).intValue() == item2.getItemId()) {
                    物品2位置 = item2.getPosition();
                    if (物品1位置 == 物品2位置) {
                        passedMap.remove(item2);
                        break;
                    }
                    MapleInventoryManipulator.move(c, Type, 物品1位置, 物品2位置);
                    passedMap.remove(item2);
                    位置交換 = true;
                    break;
                }
            ...
            if (位置交換) {
                for (final IItem item2 : passedMap) {
                    if (物品1位置 == item2.getPosition()) {
                        item2.setPosition(物品2位置);
                    }
                }
            }
        }
                ...
                if (toUse.getItemId() != 2022428) {
                    ...
                    if (rewards != null && ((Integer)rewards.getLeft()).intValue() > 0) {
                        ...
                        while (!rewarded) {
                            for (final StructRewardItem reward : rewards.getRight()) {
                                if (reward.getProb() > 0 && Randomizer.nextInt(((Integer)rewards.getLeft()).intValue()) < reward.getProb()) {
                    ...
                    switch (toUse.getItemId()) {
                        case 2022428: {
                            ...
                            final int reward2 = RandomRewards.getJxBoxReward();
                            final String box = "新年礼盒";
                            int amount = 1;
                            switch (reward2) {
                                case 2000004: {
                                    amount = 200;
                            ...
                            if (chr.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.USE).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.SETUP).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.ETC).getNextFreeSlot() > -1) {
                                ...
                                if (rareness > 0) {
                                    Broadcast.broadcastMessage(MaplePacketCreator.getGachaponMega("\t恭喜\u0010 " + c.getPlayer().getName(), " : 从" + box + "抽到！", item2, rareness));
                                }
                            }
                            else {
            ...
            while (eqq.getState() != new_state) {
                for (int i = 0; i < lines; ++i) {
                    for (boolean rewarded = false; !rewarded; rewarded = true) {
                        final StructPotentialItem pot = (StructPotentialItem)(pots.get(Randomizer.nextInt(pots.size()))).get(reqLevel);
    ...
    public static final boolean UseUpgradeScroll(final byte slot, final byte dst, final byte ws, final MapleClient c, final MapleCharacter chr, final int vegas) {
        ...
        if (!GameConstants.isSpecialScroll(scroll.getItemId()) && !GameConstants.isCleanSlate(scroll.getItemId()) && !GameConstants.isEquipScroll(scroll.getItemId()) && !GameConstants.isPotentialScroll(scroll.getItemId())) {
            if (toScroll.getUpgradeSlots() < 1 && scroll.getItemId() != 2049035 && scroll.getItemId() != 2049118) {
                c.sendPacket(MaplePacketCreator.getInventoryFull());
                return false;
            }
        }
        ...
        if (scrollReqs.size() > 0 && !scrollReqs.contains(Integer.valueOf(toScroll.getItemId())) && !GameConstants.isJh(scroll.getItemId())) {
            c.sendPacket(MaplePacketCreator.getInventoryFull());
            return false;
        }
        if (whiteScroll) {
        ...
        if (scroll.getItemId() == 2049118 && toScroll.getMpR() > 0) {
            chr.dropMessage(5, "当前装备已经上过防爆卷轴了,还是用他做啥.");
            c.sendPacket(MaplePacketCreator.enableActions());
            return false;
        }
        if (scroll.getItemId() == 2049117) {
            final int[] 枫叶套装 = { 1302020, 1332020, 1382009, 1452016, 1462014, 1472030, 1482020, 1492020, 1302030, 1332025, 1382012, 1412011, 1422014, 1432012, 1442024, 1452022, 1462019, 1472032, 1482021, 1492021, 1302064, 1312032, 1322054, 1412027, 1422029, 1432040, 1442051, 1332055, 1332056, 1372034, 1382039, 1452045, 1462040, 1472055, 1482022, 1492022, 1002508, 1002509, 1002510, 1002511, 1003224, 1102071, 1082252, 1112444, 1012098, 1012101, 1012102, 1012103, 1032035, 1122015, 1122074, 1092030, 1092045, 1092046, 1092047 };
            boolean pd = false;
            for (int a = 0; a < 枫叶套装.length; ++a) {
                if (toScroll.getItemId() == 枫叶套装[a]) {
                    pd = true;
                }
            }
            if (!pd) {
                chr.dropMessage(5, "当前卷轴只能给制作类里面的新手枫叶装备使用");
                c.sendPacket(MaplePacketCreator.enableActions());
                return false;
            }
        }
        if (GameConstants.isJh(scroll.getItemId())) {
            final int[] 进化戒指 = { 1113045, 1113049, 1113046, 1113050, 1113047, 1113051, 1113129, 1113130, 1113131, 1113232, 1113233, 1113234 };
            boolean pd = false;
            for (int a = 0; a < 进化戒指.length; ++a) {
                if (toScroll.getItemId() == 进化戒指[a]) {
                    pd = true;
                }
            }
            if (!pd) {
                chr.dropMessage(5, "当前卷轴只能给进化戒指使用");
                c.sendPacket(MaplePacketCreator.enableActions());
                return false;
            }
        }
        final IEquip scrolled = (IEquip)ii.scrollEquipWithId((IItem)toScroll, scroll, whiteScroll, chr, vegas, checkIfGM);
        ...
        else if (scrolled.getLevel() == 0 && scroll.getItemId() == 2049035) {
            scrollSuccess = ScrollResult.SUCCESS;
        }
        else if (scroll.getItemId() == 2049118) {
            scrollSuccess = ScrollResult.SUCCESS;
        }
        else if (scroll.getItemId() == 2049117) {
            scrollSuccess = ScrollResult.SUCCESS;
        }
        else if (GameConstants.isJh(scroll.getItemId())) {
            scrollSuccess = ScrollResult.SUCCESS;
        }
        else {
    ...
    public static void UsePenguinBox(final LittleEndianAccessor slea, final MapleClient c) {
        final List<Integer> gift = (List<Integer>)new ArrayList();
        final byte slot = (byte)slea.readShort();
        ...
        final String 道具代码 = "" + toUse.getItemId();
        if (((Integer)Start.ConfigValuesMap.get("自定义箱子代码")).intValue() > 0) {
            c.getSession().writeAndFlush(MaplePacketCreator.enableActions());
            NPCScriptManager.getInstance().start(c, 9010000, 道具代码);
        }
        else {
            ...
            switch (item) {
                case 2022570: {
                    gift.add(Integer.valueOf(1302119));
                    gift.add(Integer.valueOf(1312045));
                    gift.add(Integer.valueOf(1322073));
                    break;
                }
                case 2022571: {
                    gift.add(Integer.valueOf(1372053));
                    gift.add(Integer.valueOf(1382070));
                    break;
                }
                case 2022572: {
                    gift.add(Integer.valueOf(1462066));
                    gift.add(Integer.valueOf(1452073));
                    break;
                }
                case 2022573: {
                    gift.add(Integer.valueOf(1332088));
                    gift.add(Integer.valueOf(1472089));
                    break;
                }
                case 2022575: {
                    gift.add(Integer.valueOf(1040145));
                    gift.add(Integer.valueOf(1041148));
                    break;
                }
                case 2022576: {
                    gift.add(Integer.valueOf(1050155));
                    gift.add(Integer.valueOf(1051191));
                    break;
                }
                case 2022577: {
                    gift.add(Integer.valueOf(1040146));
                    gift.add(Integer.valueOf(1041149));
                    break;
                }
                case 2022578: {
                    gift.add(Integer.valueOf(1040147));
                    gift.add(Integer.valueOf(1041150));
                    break;
                }
                case 2022580: {
                    gift.add(Integer.valueOf(1072399));
                    gift.add(Integer.valueOf(1060134));
                    gift.add(Integer.valueOf(1061156));
                    break;
                }
                case 2022581: {
                    gift.add(Integer.valueOf(1072400));
                    break;
                }
                case 2022582: {
                    gift.add(Integer.valueOf(1072401));
                    gift.add(Integer.valueOf(1060135));
                    gift.add(Integer.valueOf(1061157));
                    break;
                }
                case 2022583: {
                    gift.add(Integer.valueOf(1072402));
                    gift.add(Integer.valueOf(1060136));
                    gift.add(Integer.valueOf(1061158));
                    break;
                }
            }
        }
        if (gift.isEmpty()) {
            if (c.getPlayer().isGM()) {
                c.getPlayer().dropMessage(6, "箱子代码是:" + item);
            }
        }
        else {
            final int rand = ThreadLocalRandom.current().nextInt(gift.size());
            MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, (short)slot, (short)1, false);
            MapleInventoryManipulator.addById(c, ((Integer)gift.get(rand)).intValue(), (short)1);
            gift.clear();
        }
        c.getSession().write(MaplePacketCreator.enableActions());
    }
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId && (c.getPlayer().getMapId() < 910000000 || c.getPlayer().getMapId() > 910000022)) {
            ...
            if (c.getPlayer().isGM() || !FieldLimitType.SummoningBag.check(chr.getMap().getFieldLimit())) {
                ...
                for (final Pair<Integer, Integer> toSpawn2 : toSpawn) {
                    if (Randomizer.nextInt(99) <= ((Integer)toSpawn2.getRight()).intValue()) {
                        final MapleMonster ht = MapleLifeFactory.getMonster(((Integer)toSpawn2.getLeft()).intValue());
    ...
    public static final void UseTreasureChest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getInventory(MapleInventoryType.CASH).countById(keyIDforRemoval) > 0) {
            if (chr.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.USE).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.SETUP).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.ETC).getNextFreeSlot() > -1) {
                ...
                if (rareness > 0) {
                    Broadcast.broadcastMessage(MaplePacketCreator.getGachaponMega("\t恭喜\u0010 " + c.getPlayer().getName(), " : 从" + box + "抽到！", item, rareness));
                }
            }
            else {
                chr.dropMessage(5, "你有一个栏位满了，请空出来再打开" + box + "！");
    ...
    public static final void UseCashItem(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (itemId) {
                ...
                if (c.getPlayer().getQuest(quest).getStatus() == 1 && quest.canComplete(c.getPlayer(), Integer.valueOf(npcid))) {
                    ...
                    if (mapId != -1) {
                        ...
                        if (map2.containsNPC(npcid) && !FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit()) && !FieldLimitType.VipRock.check(map2.getFieldLimit()) && c.getPlayer().getEventInstance() == null) {
                            ...
                            c.getPlayer().dropMessage(1, "当前地图无法使用该功能。");
                            used = false;
                        }
                    }
                    else {
            ...
            case 5050000: {
                final Map<MapleStat, Long> statupdate = new EnumMap(MapleStat.class);
                ...
                if (used) {
                    switch (apto) {
                        ...
                        case MAXHP: {
                            ...
                            statupdate.put(MapleStat.MAXHP, Long.valueOf((int)maxhp));
                            break;
                        }
                        case MAXMP: {
                            ...
                            statupdate.put(MapleStat.MAXMP, Long.valueOf((int)maxmp));
                            break;
                        }
                    }
                    switch (apfrom) {
                        ...
                        case MAXHP: {
                            ...
                            playerst.setMaxHp(maxhp);
                            statupdate.put(MapleStat.MAXHP, Long.valueOf((int)maxhp));
                            statupdate.put(MapleStat.HP, Long.valueOf((int)playerst.getHp()));
                            break;
                        }
                        case MAXMP: {
                            short maxmp = playerst.getMaxMp();
                            ...
                            playerst.setMaxMp(maxmp);
                            statupdate.put(MapleStat.MAXMP, Long.valueOf((int)maxmp));
                            statupdate.put(MapleStat.MP, Long.valueOf((int)playerst.getMp()));
                            break;
                        }
                    }
            ...
            case 5570000: {
                ...
                if (((Integer)Start.ConfigValuesMap.get("金锤子使用开关")).intValue() <= 0) {
                    c.getPlayer().dropMessage(5, "管理员已经在后台关闭了这个功能");
                    break;
                }
                if (item3 == null) {
                    break;
                ...
                final int 随机数 = (int)Math.floor(Math.random() * 100.0);
                    if (随机数 <= ((Integer)Start.ConfigValuesMap.get("金锤子使用概率")).intValue()) {
                        ...
                        c.getPlayer().getClient().sendPacket(MTSCSPacket.sendHammer效果(true, item3.getViciousHammer()));//成功
                    }else {
                        MapleInventoryManipulator.removeById(c, MapleInventoryType.CASH, itemId, 1, true, false);//扣掉金锤子
                        c.getPlayer().getClient().sendPacket(MTSCSPacket.sendHammer效果(false, item3.getViciousHammer()));//失败
            ...
            case 5077000: {
                ...
                final StringBuilder sb = new StringBuilder();
                addMedalString(c.getPlayer(), sb);
                final List<String> messages = (List<String>)new LinkedList();
            ...
            case 5152107: {
                ...
                if (color >= 0) {
                    if (changeFace(c.getPlayer(), color)) {
                        ...
                        c.getPlayer().dropMessage(1, "使用日拋隐形眼鏡出现错误。");
                    }
                }
                else {
                    c.getPlayer().dropMessage(1, "使用日拋隐形眼鏡出现错误。");
            ...
            case 5170000: {
                ...
                if (c.getPlayer().getSummonedPets().size() != 1) {
                    c.getPlayer().dropMessage(1, "只有开启一隻宠物的时候才能使用。");
                    used = false;
                    break;
                }
            ...
            case 5240028: {
                ...
                if (pet.getCloseness() < 30000) {
                    ...
                    if (pet.getCloseness() >= GameConstants.getClosenessNeededForLevel(pet.getLevel() + 1) && pet.getLevel() < 30) {
            ...
            case 5450000: {
                for (final int k : GameConstants.blockedMaps) {
                    if (c.getPlayer().getMapId() == k) {
    ...
    public static final void PlayerPickup(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            else if (chr.getPosition().distanceSq((Point2D)mapitem.getPosition()) > 640000.0 && ((Integer)Start.ConfigValuesMap.get("吸物检测")).intValue() > 0) {
            ...
            FileoutputUtil.outError("logs/捡取道具异常.txt", (Throwable)ex);
        }
        finally {
            lock.unlock();
        }
    ...
    public static final void PetPickup(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            if (count > range && ((Integer)Start.ConfigValuesMap.get("吸物检测")).intValue() > 0) {
    ...
    public static final void OwlWarp(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (!c.getPlayer().isAlive()) {
            c.getSession().write(MaplePacketCreator.getOwlMessage(4));
            return;
        }
        if (c.getPlayer().getTrade() != null) {
            c.getSession().write(MaplePacketCreator.getOwlMessage(7));
            return;
        }
        final int id = slea.readInt();
        final int map = slea.readInt();
        if (map == 1000000000) {
            c.getSession().write(MaplePacketCreator.getOwloff());
            c.getPlayer().jyhbh = id;
            NPCScriptManager.getInstance().start(c, 2001, "交易行购买");
            return;
        }
        if (map != 2000000000) {
            if (c.getPlayer().getMapId() >= 910000000 && c.getPlayer().getMapId() <= 910000022 && c.getPlayer().getPlayerShop() == null) {
                if (map >= 910000001 && map <= 910000022) {
                    c.getSession().write(MaplePacketCreator.getOwlMessage(0));
                    ...
                    if (merchant != null) {
                        if (merchant.isOwner(c.getPlayer())) {
                            ...
                            c.getPlayer().dropMessage(1, "这个商店在整理或者是沒在販卖東西。");
                        }
                    ...
                    c.getSession().write(MaplePacketCreator.getOwlMessage(23));
                }
            }
            else {
                c.getSession().write(MaplePacketCreator.getOwlMessage(23));
        ...
        c.getSession().write(MaplePacketCreator.getOwloff());
        c.getPlayer().jyhbh = id;
        if (c.getPlayer().jyhbh < 0) {
            c.getPlayer().dropMessage("编号非法！");
            return;
        }
        NPCScriptManager.getInstance().start(c, 9900004, "批量放大镜");
    }
    ...
    public static final void QuestMaplepoints(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
