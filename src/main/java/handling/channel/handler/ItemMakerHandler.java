...
public class ItemMakerHandler
{
    public static final void ItemMaker(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (makerType) {
            ...
            case 3: {
                ...
                final int level = MapleItemInformationProvider.getInstance().getItemMakeLevel(etc);
                if (level <= 30) {
                    c.getPlayer().dropMessage(1, "当前道具等级小于30级能合成什么鬼东西");
                    break;
                }
    ...
    private static void addEnchantStats(final Map<String, Byte> stats, final Equip item) {
        int s = ((Byte)stats.get("incPAD")).byteValue();
        if (s != 0) {
            item.setWatk((short)(item.getWatk() + s));
        }
        s = ((Byte)stats.get("incMAD")).byteValue();
        if (s != 0) {
            item.setMatk((short)(item.getMatk() + s));
        }
        s = ((Byte)stats.get("incACC")).byteValue();
        if (s != 0) {
            item.setAcc((short)(item.getAcc() + s));
        }
        s = ((Byte)stats.get("incEVA")).byteValue();
        if (s != 0) {
            item.setAvoid((short)(item.getAvoid() + s));
        }
        s = ((Byte)stats.get("incSpeed")).byteValue();
        if (s != 0) {
            item.setSpeed((short)(item.getSpeed() + s));
        }
        s = ((Byte)stats.get("incJump")).byteValue();
        if (s != 0) {
            item.setJump((short)(item.getJump() + s));
        }
        s = ((Byte)stats.get("incMaxHP")).byteValue();
        if (s != 0) {
            item.setHp((short)(item.getHp() + s));
        }
        s = ((Byte)stats.get("incMaxMP")).byteValue();
        if (s != 0) {
            item.setMp((short)(item.getMp() + s));
        }
        s = ((Byte)stats.get("incSTR")).byteValue();
        if (s != 0) {
            item.setStr((short)(item.getStr() + s));
        }
        s = ((Byte)stats.get("incDEX")).byteValue();
        if (s != 0) {
            item.setDex((short)(item.getDex() + s));
        }
        s = ((Byte)stats.get("incINT")).byteValue();
        if (s != 0) {
            item.setInt((short)(item.getInt() + s));
        }
        s = ((Byte)stats.get("incLUK")).byteValue();
        if (s != 0) {
            item.setLuk((short)(item.getLuk() + s));
        }
        s = ((Byte)stats.get("randOption")).byteValue();
        if (s > 0) {
        ...
        s = ((Byte)stats.get("randStat")).byteValue();
        if (s > 0) {
            final boolean success = Randomizer.nextBoolean();
    ...
    private static int getRandomGem(final List<Pair<Integer, Integer>> rewards) {
        final List<Integer> items = (List<Integer>)new ArrayList();
