    ...
    public static final void MonsterCarnival(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (tab) {
            case 0: {
                ...
                if (num >= mobs.size() || c.getPlayer().getAvailableCP() < ((Integer)(mobs.get(num)).right).intValue()) {
                    c.getPlayer().dropMessage(5, "你沒有足够的 CP.");
                    c.sendPacket(MaplePacketCreator.enableActions());
                    return;
                }
                final MapleMonster mons = MapleLifeFactory.getMonster(((Integer)(mobs.get(num)).left).intValue());
                if (mons != null && c.getPlayer().getMap().makeCarnivalSpawn(c.getPlayer().getCarnivalParty().getTeam(), mons, num)) {
                    c.getPlayer().getCarnivalParty().useCP(c.getPlayer(), ((Integer)(mobs.get(num)).right).intValue());
            ...
            case 1: {
                ...
                final MCSkill skil = MapleCarnivalFactory.getInstance().getSkill(((Integer)skillid.get(num)).intValue());
