...
public class PlayersHandler
{
    ...
    public static void GiveFame(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (target != null) {
            if (target.getId() == chr.getId()) {
                ...
                Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封锁系统] " + chr.getName() + " 因为修改封包而被管理员永久停权。"));
                Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语]  " + chr.getName() + "(" + chr.getId() + ") 修改名声封包，使用时封锁。加自己名声"));
                chr.ban("修改封包", true, true, false);
                chr.getClient().getSession().close();
                return;
            }
            if (chr.getLevel() < 15) {
                FileoutputUtil.logToFile("logs/Hack/Ban/修改封包.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家：" + chr.getName() + "(" + chr.getId() + ")(等级:" + (int)chr.getLevel() + ") 修改名声封包，使用时封锁。十五等以下加名声");
                Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封锁系统] " + chr.getName() + " 因为修改封包而被管理员永久停权。"));
                Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语]  " + chr.getName() + "(" + chr.getId() + ")(等级:" + (int)chr.getLevel() + ") 修改名声封包，使用时封锁。十五等以下加名声"));
                chr.ban("修改封包", true, true, false);
                chr.getClient().getSession().close();
                return;
            }
    ...
    public static void ChatRoomHandler(final LittleEndianAccessor slea, final MapleClient c) {
        int 人物经验 = Integer.parseInt(ServerProperties.getProperty("Lzj.expRate"));
        final int 人物物品爆率 = ChannelServer.getInstance(c.getChannel()).getDropRate() * MapleParty.活动爆率倍率;
        final int 人物金币爆率 = ChannelServer.getInstance(c.getChannel()).getMesoRate() * MapleParty.活动金币倍率;
        if (((Integer)Start.ConfigValuesMap.get("阶段经验开关")).intValue() > 0) {
            for (int i = 0; i < Start.exptable.size(); ++i) {
                if (c.getPlayer().getLevel() <= Integer.valueOf((String)(Start.exptable.get(i)).getLeft()).intValue()) {
                    人物经验 = ((Integer)(Start.exptable.get(i)).getRight()).intValue();
                    break;
                }
            }
        }
        ...
        c.getPlayer().dropMessage(6, "当前全服怪物经验 " + Math.round((float)人物经验) * MapleParty.活动经验倍率 + "倍,物品爆率 " + Math.round((float)人物物品爆率) + "倍, 金币爆率 " + Math.round((float)人物金币爆率) + "倍,");
        c.getPlayer().dropMessage(6, "当前个人经验倍率 " + (long)(Math.round((float)c.getPlayer().getEXPMod()) * 100) * Math.round(c.getPlayer().getStat().expBuff / 100.0) + "%, 爆率 " + (long)(Math.round((float)c.getPlayer().getDropMod()) * 100) * Math.round(c.getPlayer().getStat().dropBuff / 100.0) + "%, 金币倍率 " + Math.round(c.getPlayer().getStat().mesoBuff / 100.0) * 100L + "%");
    ...
    public static void TouchReactor(final LittleEndianAccessor slea, final MapleClient c) {
        final int oid = slea.readInt();
        final boolean touched = slea.available() == 0L || slea.readByte() > 0;
        final MapleReactor reactor = c.getPlayer().getMap().getReactorByOid(oid);
        if (!touched || reactor == null || !reactor.isAlive()) {
            return;
        }
        if (c.getPlayer().isAdmin()) {
            c.getPlayer().dropMessage(5, "反应堆信息 - oid: " + oid + " isTimerActive: " + reactor.isTimerActive() + " ReactorType: " + reactor.getReactorType());
        }
        ...
        if (mode == 0) {
            ...
            if (c.getPlayer().getMarriageId() > 0) {
                ...
                if (c.getPlayer().haveItem(newItemId)) {
                    c.getPlayer().dropMessage("请先将身上的戒指丟弃写。");
                    c.sendPacket(MaplePacketCreator.enableActions());
                    return;
                }
                if (chr == null) {
    ...
    public static void LieDetectorResponse(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (answer.equalsIgnoreCase(ld.getAnswer())) {
            ...
            if (search_chr != null && search_chr.getId() != c.getPlayer().getId()) {
                search_chr.dropMessage(1, c.getPlayer().getName() + " 通过测谎仪的检测。");
            }
            ld.end();
            c.getSession().writeAndFlush(MaplePacketCreator.LieDetectorResponse((byte)9, (byte)0));
            Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 玩家: " + c.getPlayer().getName() + " (等级 " + (int)c.getPlayer().getLevel() + ") 通过了测谎仪检测。"));
        }
            ...
            final MapleCharacter search_chr = c.getPlayer().getMap().getCharacterByName(ld.getTester());
            if (search_chr != null && search_chr.getId() != c.getPlayer().getId()) {
                search_chr.dropMessage(1, c.getPlayer().getName() + " 沒有通过测谎仪检测。");
            }
            ld.end();
            ...
            Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] 玩家: " + c.getPlayer().getName() + " (等级 " + (int)c.getPlayer().getLevel() + ") 未通过测谎仪检测，疑似使用脚本外挂！"));
        }
    }
}
