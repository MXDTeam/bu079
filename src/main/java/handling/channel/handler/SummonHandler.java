...
public class SummonHandler
{
    ...
    public static void SummonAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        final List<SummonAttackEntry> allDamage = (List<SummonAttackEntry>)new ArrayList();
        chr.getCheatTracker().checkSummonAttack();
        double SUMRange = 400000.0;
        if (((Integer)Start.ConfigValuesMap.get("重置技能范围开关")).intValue() > 0) {
            SUMRange = (double)((Integer)Start.ConfigValuesMap.get("重置技能总范围")).intValue();
        }
        long atk = 199999L;
        if (((Integer)Start.ConfigValuesMap.get("伤害检测")).intValue() > 0) {
            atk = (long)((Integer)Start.ConfigValuesMap.get("伤害上限")).intValue();
        }
        for (int i = 0; i < numAttacked; ++i) {
            final MapleMonster mob = map.getMonsterByOid(slea.readInt());
            if (mob != null) {
                if (chr.getPosition().distanceSq((Point2D)mob.getPosition()) > SUMRange) {
        ...
        for (final SummonAttackEntry attackEntry : allDamage) {
            ...
            if (toDamage > 0 && summonEffect.getMonsterStati().size() > 0 && summonEffect.makeChanceResult()) {
                for (final Entry<MonsterStatus, Integer> z : summonEffect.getMonsterStati().entrySet()) {
                    mob2.applyStatus(chr, new MonsterStatusEffect((MonsterStatus)z.getKey(), (Integer)z.getValue(), summonSkill.getId(), null, false), summonEffect.isPoison(), 4000L, true, summonEffect);
                }
            }
            if (chr.isGM() || (long)toDamage < atk) {
