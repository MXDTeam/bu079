...
public class PlayerHandler{
    ...
    public static final void UseChair(final int itemId, final MapleClient c, final MapleCharacter chr) {
        ...
        if (toUse == null && itemId >= 3010000 && itemId < 9999999) {
            FileoutputUtil.logToFile("logs/Hack/Ban/修改封包.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家：" + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") 修改椅子(" + itemId + ")封包，坐上椅子时封锁。 身上并沒有该物品");
            Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封锁系统] " + c.getPlayer().getName() + " 因为修改封包而被管理员永久停权。"));
            Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语]  " + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") 修改椅子(" + itemId + ")封包，坐上椅子时封锁。 身上并沒有该物品"));
        ...
        boolean 判定 = false;
        int tamingMob = 0;
        if (PlayerHandler.特殊椅子开关) {
            for (int i = 0; i < PlayerHandler.特殊椅子.length; ++i) {
                if (itemId == Integer.parseInt(PlayerHandler.特殊椅子[i])) {
                    tamingMob = Integer.parseInt(PlayerHandler.特殊坐骑[i]);
                    判定 = true;
                }
            }
        }
        if (判定) {
            SkillFactory.getSkill(20001019).getEffect(1).applyTo(chr);
            c.sendPacket(MaplePacketCreator.enableActions());
        }
        else {
    ...
    public static final void CharInfoRequest(final int objectid, final MapleClient c, final MapleCharacter chr) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null || chr == null) {
            return;
        }
        if (c.getPlayer().isGM()) {
            if (c.getPlayer().hasEquipped(1003824)) {
                final String guildName = NPCConversationManager.获取家族名称(c.getPlayer().getMap().getCharacterById(objectid).getGuildId());
    ...
    public static final void TakeDamage(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (damage > 0) {
            ...
            if (type != -2 && type != -3 && type != -4) {
                final int bouncedam_ = ((Randomizer.nextInt(100) < chr.getStat().DAMreflect_rate) ? chr.getStat().DAMreflect : 0) + ((type == -1 && chr.getBuffedValue(MapleBuffStat.POWERGUARD) != null) ? chr.getBuffedValue(MapleBuffStat.POWERGUARD).intValue() : 0);
            ...
            if (type != -2 && type != -3 && type != -4) {
                switch (chr.getJob()) {
                    case 112: {
                        ...
                        if (chr.getSkillLevel(skill2) > 0) {
                            damage = (int)((double)damage - (1.0 - (double)skill2.getEffect((int)chr.getSkillLevel(skill2)).getX() / 1000.0) * (double)damage);
                            break;
                        }
                        break;
                    }
                ...
                if (chr.getBuffedValue(MapleBuffStat.COMBO_BARRIER) != null) {
                    damage = (int)((1.0 - (double)chr.getBuffedSkill_X(MapleBuffStat.COMBO_BARRIER).intValue() / 100.0) * (double)damage);
                }
            }
            if (chr.getBuffedValue(MapleBuffStat.MAGIC_GUARD) != null) {
                ...
                final int mesoloss = (int)((double)damage * (chr.getBuffedValue(MapleBuffStat.MESOGUARD).doubleValue() / 100.0));
    ...
    public static final void closeRangeAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr, final boolean energy) {
        ...
        if (!energy) {
            ...
            if (numFinisherOrbs > 0) {
                ...
                if (c.getPlayer().getSkillLevel(combo) > 0) {
                    maxdamage *= 1.0 + ((double)combo.getEffect((int)c.getPlayer().getSkillLevel(combo)).getDamage() / 100.0 - 1.0) * (double)(comboBuff.intValue() - 1);
                }
            }
            if (isFinisher(attack.skill)) {
                if (numFinisherOrbs == 0) {
       ...
       if(c.getPlayer().吸怪特权 == 1&&c.getPlayer().jishu==5){
        ...
        final AttackInfo attack2 = DamageParse.DivideAttack(attack, chr.isGM() ? chr.getCloneDamgeRate() : chr.getCloneDamgeRate());
    ...
    public static final void DropMeso(final int meso, final MapleCharacter chr) {
        if (((Integer)Start.ConfigValuesMap.get("丢出金币")).intValue() < 1) {
    ...
    public static final void Heal(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        if (healHP != 0 && chr.canHP()) {
            ...
            if (healHP > check * 5) {
                chr.getCheatTracker().registerOffense(CheatingOffense.REGEN_HIGH_HP, String.valueOf(healHP) + "服务端:" + stats.getHealHP());
        ...
        if (slea.available() != 0L) {
            ...
            if (targetid != -1 && !chr.isAlive()) {
                ...
                if (!wheel) {
                    ...
                    chr.getStat().setHp(chr.getStat().getMaxHp() * 40 / 100);
    ...
    public static void PlayerUpdate(final MapleCharacter chr) {
