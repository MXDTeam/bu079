...
public class StatsHandling
{
            ...
            if (chr.getRemainingAp() > 0) {
                switch (state) {
                    case STR: {
                        ...
                        statupdate.put(MapleStat.STR, Long.valueOf((int)stat.getStr()));
                        break;
                    }
                    case DEX: {
                        ...
                        statupdate.put(MapleStat.DEX, Long.valueOf((int)stat.getDex()));
                        break;
                    }
                    case INT: {
                        ...
                        statupdate.put(MapleStat.INT, Long.valueOf((int)stat.getInt()));
                        break;
                    }
                    case LUK: {
                        ...
                        statupdate.put(MapleStat.LUK, Long.valueOf((int)stat.getLuk()));
                        break;
                    }
                    case MAXHP: {
                ...
                statupdate.put(MapleStat.AVAILABLEAP, Long.valueOf((int)chr.getRemainingAp()));
                c.sendPacket(MaplePacketCreator.updatePlayerStats(statupdate, true, chr));
            }
        }
    }
    ...
    public static final void DistributeSP(final int skillid, final MapleClient c, final MapleCharacter chr) {
        ...
        if (skill != null) {
            ...
            int maxlevel2 = skill.isFourthJob() ? chr.getMasterLevel(skill) : skill.getMaxLevel();
            if (maxlevel2 >= 30) {
                maxlevel2 = 30;
            }
            ...
            if (remainingSp > 0 && curLevel + 1 <= maxlevel && (curLevel + 1 <= maxlevel2 || skill.getId() == 5110001) && (skill.canBeLearnedBy((int)chr.getJob()) || isBeginnerSkill)) {
                if (!isBeginnerSkill) {
    ...
    static int gainStatByType(final MapleCharacter chr, final MapleStat type, final int gain) {
        if (chr != null) {
            ...
            if (type.equals(MapleStat.STR)) {
                ...
                newVal = chr.getStat().getInt() + gain;
                if (newVal >= 32767) {
                ...
                newVal = chr.getStat().getLuk() + gain;
                if (newVal >= 32767) {
                ...
                newVal = chr.getStat().getDex() + gain;
                if (newVal >= 32767) {
            ...
            chr.updateSingleStat(type, (int)(short)newVal);
        }
        return 0;
    }
}
