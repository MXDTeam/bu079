...
public class PlayerInteractionHandler
{
    ...
    public static final byte MOVE_OMOK = 63;
        ...
        switch (action) {
            ...
            case 0: {
                ...
                if (createType == 1 || createType == 2) {
                    ...
                    for (int i = 1; i <= 1; ++i) {
                        chr.dropMessage(1, "精灵商人修复度满高，但若真发生服务器资料库产生异常时有机率吃道具\r\n并且这种若吃了一定会全部吃光\r\n建议玩家摆少量商品，卖完再去补货\r\n你认定这道具若真的被吃了也沒关系，若发生被吃装将不会给予任何补偿\r\n请同意了再摆摊商人，若不同意的玩家~请勿~摆摊，谢谢配合\r\n");
                    }
                    if (createType == 4) {
            ...
            case 13: {
                ...
                if (csInvite == 6 && type == 4 && chr.getTrade() != null && chr.getTrade().getPartner() != null) {
                    MapleTrade.visitTradea(chr, chr.getTrade().getPartner().getChr());
                    break;
                }
                if (csInvite == 6 && type != 4) {
                    if (chr.getMap() == null) {
                        return;
                    }
                    final MapleCharacter chrr = chr.getMap().getCharacterById(obid);
                    if (chrr == null || c.getChannelServer().isShutdown() || chrr.hasBlockedInventory()) {
                        c.sendPacket(MaplePacketCreator.enableActions());
                        return;
                    }
                    MapleTrade.startCS(chr);
                    MapleTrade.inviteCS(chr, chrr);
                    break;
                }
                else {
                    ...
                    if (ob2 instanceof IMaplePlayerShop && chr.getPlayerShop() == null) {
                        ...
                        if (ob2 instanceof HiredMerchant) {
                            ...
                            if (merchant2.isOwner(chr)) {
                                ...
                                chr.dropMessage(1, "这个商店在整理或者是沒在販卖東西。");
                            }
            ...
            case 6: {
                ...
                if (chr.getPlayerShop() != null) {
                    ...
                    ips4.getMessages().add(new Pair(chr.getName() + " : " + message, Byte.valueOf(ips4.getVisitorSlot(chr))));
            ...
            case 10: {
                if (chr.getTrade() != null) {
                    ...
                    final List<BoughtItem> boughtItemsA = chr.getPlayerShop().getBoughtItems();
                    if (ips != null && ips.getShopType() == 1 && ips.isOwner(chr) && ips.isAvailable() && boughtItemsA.size() == 0) {
                        c.sendPacket(PlayerShopPacket.shopErrorMessage(21, 0));
                        c.sendPacket(MaplePacketCreator.serverNotice(1, "请通过弗兰德里拿回剩余物品."));
                        c.sendPacket(MaplePacketCreator.enableActions());
            ...
            case 31: {
                ...
                if (ivItem.getQuantity() >= bundles_perbundle) {
                    ...
                    if (GameConstants.isThrowingStar(ivItem.getItemId()) || GameConstants.isBullet(ivItem.getItemId())) {
                        ...
                        FileoutputUtil.logToFile("logs/Data/精灵商人放入道具.txt", FileoutputUtil.NowTime() + "账号角色名字:" + c.getAccountName() + " " + c.getPlayer().getName() + " 道具： " + ivItem.getItemId() + " 数量:  " + (int)bundles + "\r\n");
                    }
                    else {
            ...
            case 33: {
                ...
                if ((long)shop4.getMeso() + check3 > 2147483647L) {
                    c.getPlayer().dropMessage(1, "您购买的商店營业额已经超标，请通知店主来收钱。");
                    c.sendPacket(MaplePacketCreator.enableActions());
                    return;
                }
            ...
            case 36: {
                ...
                if (item3 != null && item3.bundles > 0) {
                    ...
                    FileoutputUtil.logToFile("logs/Data/精灵商人道具拿出.txt", FileoutputUtil.NowTime() + "账号角色名字:" + c.getAccountName() + " " + c.getPlayer().getName() + " 道具： " + item_get.getItemId() + " 数量:  " + (int)item_get.getQuantity() + "\r\n");
                }
            ...
            case 38: {
                ...
                for (int j = 0; j < imps.getItems().size(); ++j) {
                    if (((MaplePlayerShopItem)imps.getItems().get(j)).bundles == 0) {
                ...
                FileoutputUtil.logToFile("logs/Data/精灵商人整理道具.txt", FileoutputUtil.NowTime() + "账号角色名字:" + c.getAccountName() + " " + c.getPlayer().getName() + " 获得金币： " + imps.getMeso() + "\r\n");
            ...
            case 39: {
                    ...
                    try {
                        ...
                        if (merchant4 != null && merchant4.getShopType() == 1 && merchant4.isOwner(chr) && merchant4.isAvailable()) {
                            c.sendPacket(PlayerShopPacket.shopErrorMessage(21, 0));
                            c.sendPacket(MaplePacketCreator.serverNotice(1, "请去找弗兰德里领取你的装备和金币"));
                            c.sendPacket(MaplePacketCreator.enableActions());
            ...
            case 50: {
                c.getPlayer().dropMessage(5, "放弃是不被允许的");
                c.sendPacket(MaplePacketCreator.enableActions());
                break;
            }
            case 59: {
            ...
            case 48: {
                c.getPlayer().dropMessage(5, "发送平局");
                final IMaplePlayerShop ips = chr.getPlayerShop();
                if (ips == null || !(ips instanceof MapleMiniGame)) {
                    break;
                }
            ...
            case 49: {
                c.getPlayer().dropMessage(5, "男人之间的战争,不存在平局");
                c.sendPacket(MaplePacketCreator.enableActions());
                break;
            }
            case 53: {
            ...
            case 67: {
                ...
                if (slea.readByte() != game2.getTurn()) {
                    game2.broadcastToVisitors(PlayerShopPacket.shopChat("不能放在通过 " + chr.getName() + ". 失败者: " + game2.getLoser() + " 游客: " + (int)game2.getVisitorSlot(chr) + " 是否为真: " + game2.getTurn(), (int)game2.getVisitorSlot(chr)));
                    return;
                }
