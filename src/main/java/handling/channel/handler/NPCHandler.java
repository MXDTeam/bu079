...
public class NPCHandler
{
    ...
    public static final void Storage(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (((Integer)Start.ConfigValuesMap.get("游戏仓库")).intValue() < 1) {
        ...
        switch (mode) {
            case 4: {
                ...
                if (chr.getMapId() == 910000000 && chr.getMeso() < 1000) {
                    storage.store(item);
                    chr.saveToDB(false, false);
                    chr.dropMessage(1, "你沒有足够的金币放仓库道具。");
                    break;
                }
            ...
            case 5: {
                ...
                storage.sendStored(c, GameConstants.getInventoryType(itemId));
                chr.saveToDB(false, false);
                FileoutputUtil.logToFile("logs/Data/仓库操作.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了仓库操作存入物品:" + item2.getItemId() + " 数量：" + (int)item2.getQuantity());
                Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语]  账号: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了仓库操作存入物品:" + item2.getItemId() + " 数量：" + (int)item2.getQuantity()));
                break;
            }
            case 6: {
                storage.arrange();
    ...
    public static final void repairAll(final MapleClient c) {
        ...
        int price = 0;
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        final Map<Equip, Integer> eqs = (Map<Equip, Integer>)new ArrayMap();
        ...
        for (final MapleInventoryType type : array) {
            for (final IItem item : c.getPlayer().getInventory(type)) {
                if (item instanceof Equip) {
                    ...
                    if (((Integer)eqStats.get("durability")).intValue() <= 0 || eq.getDurability() >= ((Integer)eqStats.get("durability")).intValue()) {
                        continue;
                    }
                    final double rPercentage = 100.0 - Math.ceil((double)eq.getDurability() * 1000.0 / ((double)((Integer)eqStats.get("durability")).intValue() * 10.0));
    ...
    public static final void repair(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (eq.getDurability() < 0 || ((Integer)eqStats.get("durability")).intValue() <= 0 || eq.getDurability() >= ((Integer)eqStats.get("durability")).intValue()) {
            return;
        }
        final double rPercentage = 100.0 - Math.ceil((double)eq.getDurability() * 1000.0 / ((double)((Integer)eqStats.get("durability")).intValue() * 10.0));
    ...
    public static final void UseItemQuest(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        for (final IItem i : c.getPlayer().getInventory(MapleInventoryType.ETC)) {
            if (i.getItemId() / 10000 == 422) {
                ...
                if (questItemInfo != null && ((Integer)questItemInfo.getLeft()).intValue() == qid && (questItemInfo.getRight()).contains(Integer.valueOf(itemId))) {
                    found = true;
                    break;
                }
                continue;
