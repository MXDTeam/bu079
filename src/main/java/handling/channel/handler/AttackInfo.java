    ...
    public final MapleStatEffect getAttackEffect(final MapleCharacter chr, int skillLevel, final ISkill skill_) {
        ...
        if (GameConstants.isLinkedSkill(this.skill)) {
            ...
            if (this.skill != skill_.getId()) {
                FileoutputUtil.logToFile("logs/Data/AttackEffect.txt", "" + FileoutputUtil.NowTime() + " 连结技能[" + this.skill + "](" + skill_.getId() + "传承) 连结技能等级:" + skillLevel + " 不在getLinkedkill清单內卻被触发, 触发者: " + chr.getName() + " 职业: " + (int)chr.getJob() + " 等级: " + (int)chr.getLevel() + "\r\n");
            }
