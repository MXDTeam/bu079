...
public class MapleServerHandler extends ChannelInboundHandlerAdapter
{
    ...
    public MapleServerHandler(final int world, final int channel) {
        this.BlockedIP = (List<String>)new ArrayList();
        this.tracker = (Map<String, Pair<Long, Byte>>)new ConcurrentHashMap();
        this.world = world;
    ...
    public void channelActive(final ChannelHandlerContext ctx) throws Exception {
        ...
        if (track == null) {
            ...
            final long difference = System.currentTimeMillis() - ((Long)track.left).longValue();
            if (difference < 2000L) {
                ++count;
            }
        ...
        if (((Integer)Start.ConfigValuesMap.get("欢迎弹窗")).intValue() > 0) {
    ...
    public void channelInactive(final ChannelHandlerContext ctx) throws Exception {
        try {
            ...
            FileoutputUtil.outError("logs/断开连接异常.txt", (Throwable)e);
        }
    ...
    public void channelRead(final ChannelHandlerContext ctx, final Object message) {
        ...
        if (ServerConfig.LOG_PACKETS) {
            ...
            final StringBuilder sb = new StringBuilder("发现未知服务端数据包 - (包头:0x" + Integer.toHexString((int)header_num) + ")");
            sb.append(":\r\n").append(HexTool.toString(packet)).append("\r\n").append(HexTool.toStringFromAscii(packet));
            FileoutputUtil.log("logs\\服务端数据包_未知.txt", sb.toString());
        }
    }
    ...
    public static final void handlePacket(final RecvPacketOpcode header, final LittleEndianAccessor slea, final MapleClient c, final boolean cs) throws Exception {
        switch (header) {
            ...
            case QUEST_MAPLEPOINTS: {
                InventoryHandler.QuestMaplepoints(slea, c, c.getPlayer());
                break;
            }
            ...
            case CLIENT_ERROR: {
                final String error = slea.readMapleAsciiString();
                try (final RandomAccessFile file = new RandomAccessFile("logs/ClientError_log.txt", "rw")) {
                    ...
                    FileoutputUtil.outputFileError("logs\\Except\\Log_Packet_Except.txt", (Throwable)ex);
                }
                break;
            }
            case SPECIAL_ATTACK: {
    ...
    static {
        ...
        blocked.addAll((Collection<? extends RecvPacketOpcode>)Arrays.asList(block));
    }
}
