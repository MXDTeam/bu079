...
public final class MapleFamily implements Serializable
{
    ...
    public MapleFamily(final int fid) {
        this.members = (Map<Integer, MapleFamilyCharacter>)new ConcurrentHashMap();
    ...
    public static final Collection<MapleFamily> loadAll() {
        final Collection<MapleFamily> ret = (Collection<MapleFamily>)new ArrayList();
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            try (final PreparedStatement ps = con.prepareStatement("SELECT familyid FROM families")) {
    ...
    public final int setRep(final int cid, int addrep, final int oldLevel) {
        ...
        if (mgc != null && mgc.getFamilyId() == this.id) {
            ...
            if (mgc.isOnline()) {
                final List<Integer> dummy = (List<Integer>)new ArrayList();
    ...
    public final MapleFamilyCharacter addFamilyMemberInfo(final MapleCharacter mc, final int seniorid, final int junior1, final int junior2) {
        ...
        for (int i = 0; i < ret.getPedigree().size(); ++i) {
            if (((Integer)ret.getPedigree().get(i)).intValue() != ret.getId()) {
                final MapleFamilyCharacter mfc = this.getMFC(((Integer)ret.getPedigree().get(i)).intValue());
                if (mfc == null) {
        ...
        while (iterator.hasNext()) {
            final int j = ((Integer)iterator.next()).intValue();
