...
public class World{
    ...
    public static List<CheaterData> getCheaters() {
        final List<CheaterData> allCheaters = (List<CheaterData>)new ArrayList();
        for (final ChannelServer cs : ChannelServer.getAllInstances()) {
    ...
    public static void registerRespawn() {
        WorldTimer.getInstance().register((Runnable)new Respawn(), (long)((Integer)Start.ConfigValuesMap.get("怪物刷新频率设定")).intValue());
    }
        ...
        if (numTimes % 100 == 0) {
            for (final MaplePet pet : chr.getSummonedPets()) {
                if (pet.getSummoned()) {
                    ...
                    if (((Integer)Start.ConfigValuesMap.get("宠物自动吃药开关")).intValue() > 0) {
                        boolean pd = false;
                        if (chr.getItemQuantity(((Integer)Start.ConfigValuesMap.get("自动吃药道具")).intValue(), false) > 0) {
                            pd = true;
                        }
                        if (pd && pet.getFullness() <= 70) {
                            boolean gainCloseness = false;
                            if (Randomizer.nextInt(99) <= 50) {
                                gainCloseness = true;
                            }
                            if (chr.getItemQuantity(2120000, false) > 0) {
                                ...
                                if (gainCloseness && pet.getCloseness() < 30000) {
                                    ...
                                    if (newCloseness >= GameConstants.getClosenessNeededForLevel(pet.getLevel() + 1) && pet.getLevel() < 30) {
                                        pet.setLevel(pet.getLevel() + 1);
                                        chr.getClient().sendPacket(PetPacket.showOwnPetLevelUp(index));
                                        chr.getMap().broadcastMessage(PetPacket.showPetLevelUp(chr, index));
                                    }
                                }
                                if (chr != null) {
                                    chr.gainItem(2120000, -1);
                                    chr.getClient().sendPacket(MaplePacketCreator.enableActions());
                                    chr.dropMessage(5, "VIP自动使用宠物疲劳药水,目前宠物疲劳:" + newFullness);
                                }
                            }
                        }
                    }
                    if (((Integer)Start.ConfigValuesMap.get("宠物不饥饿开关")).intValue() > 0) {
                        newFullness = 100;
                    }
                    if (newFullness <= 5) {
                        pet.setFullness(15);
    ...
    public static void GainNX(final int min) {
        EventTimer.getInstance().register((Runnable)new Runnable() {
            @Override
            public void run() {
                final Map<MapleCharacter, Integer> GiveList = (Map<MapleCharacter, Integer>)new HashMap();
    ...
    public static class Party
    {
        ...
        static {
            parties = (Map<Integer, MapleParty>)new HashMap();
    ...
    public static class Buddy
    {
        ...
        private static void updateBuddies(final int characterId, final int channel, final Collection<Integer> buddies, final boolean offline, final int gmLevel, final boolean isHidden) {
            for (final Integer buddy : buddies) {
                final int ch = Find.findChannel(buddy.intValue());
                if (ch > 0) {
                    final MapleCharacter chr = ChannelServer.getInstance(ch).getPlayerStorage().getCharacterById(buddy.intValue());
                    if (chr == null) {
                        continue;
                    }
    ...
    public static class Guild
    {
        ...
        public static void save() {
            System.out.println("[" + FileoutputUtil.CurrentReadable_Time() + "][信息]:储存公会资料");
            Guild.lock.writeLock().lock();
            try {
        ...
        static {
            ...
            System.out.println("[" + FileoutputUtil.CurrentReadable_Time() + "][信息]:加载家族列表名单");
            final Collection<MapleGuild> allGuilds = MapleGuild.loadAll();
            for (final MapleGuild g : allGuilds) {
                if (g.isProper()) {
                    guilds.put(Integer.valueOf(g.getId()), g);
                }
            }
        }
    }
    ...
    public static class Find
    {
        ...
        public static int findChannel(final int id) {
            ...
            Integer ret;
            try {
                ret = (Integer)Find.idToChannel.get(Integer.valueOf(id));
            }
            finally {
        ...
        public static CharacterIdChannelPair[] multiBuddyFind(final int charIdFrom, final Collection<Integer> characterIds) {
            final List<CharacterIdChannelPair> foundsChars = (List<CharacterIdChannelPair>)new ArrayList(characterIds.size());
            for (final Integer i : characterIds) {
                final Integer channel = Integer.valueOf(findChannel(i.intValue()));
                if (channel.intValue() > 0) {
                    foundsChars.add(new CharacterIdChannelPair(i.intValue(), channel.intValue()));
                }
            }
    ...
    public static class Alliance
    {
        ...
        public static List<byte[]> getAllianceInfo(final int allianceid, final boolean start) {
            final List<byte[]> ret = (List<byte[]>)new ArrayList();
            final MapleGuildAlliance alliance = getAlliance(allianceid);
            if (alliance != null) {
                if (start) {
        ...
        public static void save() {
            System.out.println("[" + FileoutputUtil.CurrentReadable_Time() + "][信息]:储存联盟资料中");
            Alliance.lock.writeLock().lock();
            try {
        ...
        static {
            alliances = new LinkedHashMap();
            lock = new ReentrantReadWriteLock();
            System.out.println("[" + FileoutputUtil.CurrentReadable_Time() + "][信息]:加载家族成员名单");
    ...
    public static class Family
    {
        ...
        public static void save() {
            System.out.println("[" + FileoutputUtil.CurrentReadable_Time() + "][信息]:储存家族资料中");
            Family.lock.writeLock().lock();
            try {
        ...
        static {
            ...
            lock = new ReentrantReadWriteLock();
            System.out.println("[" + FileoutputUtil.CurrentReadable_Time() + "][信息]:加载学院名单");
            final Collection<MapleFamily> allGuilds = MapleFamily.loadAll();
            for (final MapleFamily g : allGuilds) {
                if (g.isProper()) {
                    families.put(Integer.valueOf(g.getId()), g);
                }
            }
        }
    }
