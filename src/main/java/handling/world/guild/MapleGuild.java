...
public class MapleGuild implements Serializable
{
    ...
    public MapleGuild(final int guildid) {
        this.members = (List<MapleGuildCharacter>)new CopyOnWriteArrayList();
        this.rankTitles = new String[5];
        this.bDirty = true;
        this.proper = true;
        this.allianceid = 0;
        this.invitedid = 0;
        this.bbs = (Map<Integer, MapleBBSThread>)new HashMap();
        ...
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            rs = ps.executeQuery();
            if (!rs.first()) {
                System.err.println("公会ID：" + this.id + " 沒有成员，系统自动解散公会。");
                rs.close();
                ps.close();
                this.writeToDB(true);
                this.proper = false;
            ...
            ps.close();
            if (!leaderCheck) {
                System.err.println("会长[" + this.leader + "]沒有在公会ID为" + this.id + "的公会中，系统自动解散这个公会。");
                this.writeToDB(true);
                this.proper = false;
                return;
            }
            ...
            while (rs.next()) {
                ...
                try (final PreparedStatement pse = con.prepareStatement("SELECT * FROM bbs_replies WHERE threadid = ?")) {
                    ...
                    while (rse.next()) {
                        thread.replies.put(Integer.valueOf(thread.replies.size()), new handling.world.guild.MapleBBSThread.MapleBBSReply(thread.replies.size(), rse.getInt("postercid"), rse.getString("content"), rse.getLong("timestamp")));
                    }
                    rse.close();
                }
    ...
    public static void meso(final MapleClient c, final int npcid) {
        try {
            final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection();
            final PreparedStatement ps = con.prepareStatement("SELECT `name`,`meso`, `level`FROM characters WHERE  gm < 1 ORDER BY `meso` DESC LIMIT 50");
            final ResultSet rs = ps.executeQuery();
            c.sendPacket(MaplePacketCreator.showMesoRanks(npcid, rs));
            rs.close();
            ps.close();
            con.close();
        }
    ...
    public static void fame(final MapleClient c, final int npcid) {
        try {
            final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection();
            final PreparedStatement ps = con.prepareStatement("SELECT `name`,`fame`, `level`FROM characters WHERE  gm < 1 ORDER BY `fame` DESC LIMIT 50");
            final ResultSet rs = ps.executeQuery();
            c.sendPacket(MaplePacketCreator.showFameRanks(npcid, rs));
            rs.close();
            ps.close();
            con.close();
        }
    ...
    public static void 万能排行榜(final MapleClient c, final int npcid) {
        try {
            final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection();
            final PreparedStatement ps = con.prepareStatement("SELECT `name`,`meso`, `level`FROM characters WHERE  gm < 1 ORDER BY `meso` DESC LIMIT 50");
            final ResultSet rs = ps.executeQuery();
    ...
    public static void displayLevelRanks(final MapleClient c, final int npcid) {
        try {
            final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection();
            final PreparedStatement ps = con.prepareStatement("SELECT `name`, `level`, `meso` FROM characters WHERE  gm < 1 ORDER BY `level` DESC LIMIT 50");
            final ResultSet rs = ps.executeQuery();
            c.sendPacket(MaplePacketCreator.showLevelRanks(npcid, rs));
            rs.close();
            ps.close();
            con.close();
        }
    ...
    public static final Collection<MapleGuild> loadAll() {
        final Collection<MapleGuild> ret = (Collection<MapleGuild>)new ArrayList();
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            try (final PreparedStatement ps = con.prepareStatement("SELECT guildid FROM guilds");
                 final ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
    ...
    private void buildNotifications() {
        ...
        final List<Integer> mem = (List<Integer>)new LinkedList();
        for (final MapleGuildCharacter mgc : this.members) {
            if (!mgc.isOnline()) {
                continue;
            }
    ...
    public void setAllianceId(final int a) {
        this.allianceid = a;
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            try (final PreparedStatement ps = con.prepareStatement("UPDATE guilds SET alliance = ? WHERE guildid = ?")) {
                ps.setInt(1, a);
                ps.setInt(2, this.id);
                ps.execute();
                ps.close();
    ...
    public final void setGuildEmblem(final short bg, final byte bgcolor, final short logo, final byte logocolor) {
        ...
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            try (final PreparedStatement ps = con.prepareStatement("UPDATE guilds SET logo = ?, logoColor = ?, logoBG = ?, logoBGColor = ? WHERE guildid = ?")) {
    ...
    public final boolean increaseCapacity() {
        ...
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            try (final PreparedStatement ps = con.prepareStatement("UPDATE guilds SET capacity = ? WHERE guildid = ?")) {
    ...
    public final List<MapleBBSThread> getBBS() {
        final List<MapleBBSThread> ret = new ArrayList(this.bbs.values());
        Collections.sort(ret, new handling.world.guild.MapleBBSThread.ThreadComparator());
        return ret;
    }
    ...
    public final void addBBSReply(final int localthreadid, final String text, final int posterID) {
        final MapleBBSThread thread = (MapleBBSThread)this.bbs.get(Integer.valueOf(localthreadid));
        if (thread != null) {
            thread.replies.put(Integer.valueOf(thread.replies.size()), new handling.world.guild.MapleBBSThread.MapleBBSReply(thread.replies.size(), posterID, text, System.currentTimeMillis()));
        }
    }
